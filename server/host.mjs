...
/* ---------------- saying what is happening ----------------
   ...
   Until now this relay logged exactly one event: a refused host
   connection. Everything else — a Warden attaching, a phone
   arriving, a character submitted into the void because no deck
   was connected — happened in silence.

   That silence is expensive. "The phone says nobody picked it up
   and the Warden's screen looks fine" is not a diagnosable
   report, and it cannot become one, because the only window that
   knows the answer was not saying anything. The server can see
   both halves at once and is the only thing that can.

   Four lines, and between them they answer the only question
   anybody actually has: is there a Warden attached, and can the
   phones see it. */
...
wss.on("connection", (ws, req) => {
  ...
  if (wantsHost) {
    ...
    if (!tokenOk(params.get("token"))) {
      ...
      log("[refused]", `a Warden deck at ${ip} connected without the token (${tries + 1}/${MAX_HOST_TRIES}).`);
      log("[refused]", `The token is served to this PC only. On another device, type it in.`);
    ...
    host = ws;
    /* The line whose absence was the problem. Nothing here ever
       confirmed the deck was attached, so "no Warden connected"
       and "Warden connected, something else is wrong" looked
       identical from the server window. */
    log("[warden]", `deck attached from ${ip}. Players can now be seen.`);
    if (players.size) log("[warden]", `${players.size} phone(s) already waiting.`);
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.t === "hello") {
      ...
      log("[player]", `${me.name} joined from ${ip}.${host ? "" : "  NO WARDEN DECK ATTACHED."}`);
    ...
    if (!me.clientId) return;
      ...
      /* ============================================================
         ...
         Six players, four hours, one dead battery. This will happen
         every session and until now there was no answer: the
         character stayed claimed by a socket that was never coming
         back, and nobody else could take it.

         The rule is unchanged for a LIVE phone. You still cannot
         take a character somebody is holding — that is theft, and
         a mis-tap in a firefight must never move somebody else's
         body. What is new is that a character whose phone has gone
         is no longer held by anyone, so it can be picked up.

         Which is exactly what "dropped" means and always did. The
         server already removes a player on disconnect; the check
         below reads the live map, so an abandoned character simply
         stops appearing in it. There is no timer, no grace period
         and no "are you sure" — the phone is either on the network
         or it is not.

         AND THE TABLE IS TOLD. Announced rather than silent,
         because a character changing hands mid-session is a fact
         about the room and not a detail of the plumbing: the
         person now playing Riley is not the person who was playing
         Riley an hour ago, and everyone at the table needs to know
         that without having to ask.

         What this deliberately does NOT do is split one phone
         across two characters. That was the other option and it
         degrades worse: two sets of private whispers arriving in
         one hand, with a swap control between them, is a secret
         leak waiting for a mis-tap. Handing the body to somebody
         else's phone keeps every secret exactly where it was — with
         the character. ============================================================ */
    if (msg.t === "claim") {
      const wanted = msg.pcId || null;
      // One character per phone, and no stealing from a phone that
    ...
    if (msg.t === "submit") {
      ...
      if (!host) {
        /* This is the moment the phone reports as "nobody picked
           that up". Say it here too, with the reason, so the two
           screens tell the same story. */
        log("[dropped]", `${me.name} submitted a character, but no Warden deck is attached.`);
        log("[dropped]", `Open  ${localUrl}/?mode=host  on THIS PC.`);
        return send(ws, { t: "denied", reason: "no-warden" });
      }
    ...
    /* Taking it down again, and just as naked. If this carried an
       identity the host would learn who cleared the card, which on a
       table where the card was about one person is nearly as
       identifying as knowing who raised it. Any phone may send it.
       The port of this lives in src/net/rtcRelay.js and the two must
       agree — see tests/rtcrelay.test.js. */
    if (msg.t === "clearsafety") {
      if (!host) return;
      send(host, { t: "clearsafety" });
      return;
    }

    /* The table deciding something. Stamped, unlike the card: a vote
       with anonymous ballots cannot tell whether everybody has
       answered, and that is exactly what makes an abstention count
       as a no. See src/engine/vote.js. */
    if (msg.t === "vote") {
      if (!host) return;
      send(host, { t: "vote", clientId: me.clientId, choice: String(msg.choice || "") });
      return;
    }

    if (msg.t === "openvote") {
      if (!host) return;
      send(host, { t: "openvote", clientId: me.clientId, topic: String(msg.topic || "") });
      return;
    }

    /* Asking the situation a question, and waving off a move that
       was addressed to you. Both need the character, and both take
       it from the server's own record rather than the message. */
    if (msg.t === "look") {
      if (!host || !me.pcId) return;
      send(host, {
        t: "look", clientId: me.clientId, asPc: me.pcId,
        about: String(msg.about || "").slice(0, 200),
      });
      return;
    }
    ...
    if (msg.t === "dispute") {
      if (!host || !me.pcId) return;
      send(host, { t: "dispute", clientId: me.clientId, asPc: me.pcId, moveId: msg.moveId || null });
      return;
    }

    /* THE TABLE MAKING A FACT TRUE. The text is the player's own
       sentence and travels untouched — the relay does not read it,
       the host does not rewrite it, and engine/tableRuling.js hands
       it to makeRuling verbatim.

       `asPc` comes from the server's record rather than the message
       for the usual reason and one extra: the entire mechanism is
       that a proposer cannot second themselves, so a phone that
       could claim to be somebody else could make anything true on
       its own. Capped at MAX_RULING; the engine checks again. */
    if (msg.t === "propose") {
      if (!host || !me.pcId) return;
      send(host, {
        t: "propose", clientId: me.clientId, asPc: me.pcId,
        text: String(msg.text || "").slice(0, 400),
        scope: String(msg.scope || "room").slice(0, 12),
        subject: String(msg.subject || "").slice(0, 80),
      });
      return;
    }
    ...
    if (msg.t === "second") {
      if (!host || !me.pcId) return;
      send(host, { t: "second", clientId: me.clientId, asPc: me.pcId });
      return;
    }
    ...
    /* Ready, and go. Ported alongside src/net/rtcRelay.js — the two
       must agree. The host decides whether a start is allowed; this
       only carries the request and the identity of who asked. */
    if (msg.t === "ready") {
      if (!host) return;
      send(host, { t: "ready", clientId: me.clientId, name: me.name, ready: msg.ready !== false });
      return;
    }
    if (msg.t === "start") {
      if (!host) return;
      send(host, { t: "start", clientId: me.clientId });
      return;
    }
  ...
  ws.on("close", () => {
    if (ws === host) {
      ...
      log("[warden]", "deck disconnected. Phones have nowhere to send to.");
      return;
    }
    const gone = players.get(ws);
    players.delete(ws);
    if (gone && gone.greeted) log("[player]", `${gone.name} left. ${players.size} still here.`);
    pushRoster();
  });
});
...
server.listen(PORT, BIND, () => {
  ...
  /* THE WARDEN URL IS NOT FILED UNDER "PHONES".

     It used to be. This banner printed "For phones on the same
     wifi:" and then listed two addresses under it, the second of
     which was the Warden screen. Somebody holding a phone reads
     that heading, sees two options, and picks the one that sounds
     like the interesting half — and the server answers with
     `[auth] refused a host connection`, because the token is
     served to loopback only and a phone is not loopback.

     The player address is the only address a phone should ever
     see. The Warden's off-machine URL still exists, but it now
     lives down beside the token, which is the thing you cannot
     use it without. */
  console.log(`  WARDEN SCREEN - this PC, and nothing else:`);
  console.log(`    ${localUrl}/?mode=host`);
  console.log(`    Play.bat opens this for you.`);
  console.log(`\n  PLAYERS - phones on the same wifi. This address, nothing after it:`);
  console.log(`    ${primary}`);
  console.log(`    Do not add ?mode=host on a phone. It will be refused.`);
  ...
  if (SESSION_TOKEN) {
    ...
    console.log(`    You only need it to run the Warden screen on another`);
    console.log(`    device, at ${primary}/?mode=host - the URL and the token`);
    console.log(`    together, because neither is any use alone.`);
    console.log(`    Players never need it and never see it.`);
  } else {
