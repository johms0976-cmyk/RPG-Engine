...
*Pronouncing.* `ui/useVoice.js` reads the feed aloud through the browser's own speech
synthesiser. It runs on the device, calls nothing, and generates no word — it pronounces words a
human author already wrote. INV-1 is about the provenance of content, not the channel it arrives
on. Off by default everywhere except a local wardenless table, which is the one configuration
with provably nobody whose job it is to read the room description out.

*Quoting the player.* When the parser misses and the sentence describes the character risking
something, the director calls for a save and puts a `reason` on the prompt. That reason is the
player's own typed sentence with a leading "I try to" trimmed off — every surviving word is
theirs. Quoting a human at the table is not composition, and it is the mechanism that lets the
empty chair make the move a Warden makes most often without writing a sentence to justify it.
The verb-to-stat map itself is a fact about English, not a judgement about any ship, which is the
same line `autoDirector` draws when it refuses to derive `rolls`.
