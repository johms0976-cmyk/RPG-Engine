...
Optional, and everything in it is optional. Without it the director can only pace the room and
describe it: rungs 4 and 6 open with `if (!mod.director) return null`, so a module with no block
gets a referee that never turns a screw. Ypsilon 14's is in
`src/modules/ypsilon14/director.js` and is worth reading before writing your own.
...
### Three rules the engine enforces for you
