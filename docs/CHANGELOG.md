...
## 2.21.0 — finishing 23, and the measurement 2.20.0 would not make

Two loose ends from the previous release, both of which it named. No new items.

### 23 — the rest of it

2.20.0 shipped the seam and listed three things that were still Mothership-shaped. All three are
done.

**Character creation is declared rather than assumed.** A ruleset's `create.steps` says which parts
of creation the system has, and both creators render exactly those. The phone creator was already a
list of steps, so it needed a filter rather than a restructure; the desk creator needed its
readiness rule rewritten, which was the part that mattered — it required a loadout and every skill
point spent unconditionally, so a system without those steps would have produced a character nobody
could finish with nothing on screen explaining why. Declaring a class step with no classes is now a
validation error rather than an empty picker.

Two more Mothership facts came out of the markup while doing it: "6d10 per Stat, 30 is about
average" is now the ruleset's own sentence, and the class picker's save summary is built from the
ruleset's save list rather than naming Sanity, Fear, Body and Armor.

**A system with no panic table has no panic.** `queuePanic` refuses once, at the top, so all
seventeen call sites — critical failure, big hit, crew death, first contact — keep firing
harmlessly. Seventeen guards is sixteen opportunities to forget one.

**The Teamster reroll is found by its ability key**, `ability: "panicReroll"`, which has been in the
class data since 2.0 and which nothing read. A ruleset whose equivalent class had any other name was
silently getting no reroll.

**Modules say which system they were written for.** `ruleset: "mothership1e"`, now declared by all
four shipped modules and by anything written in the browser editor. Stated and wrong is a problem
and the library card refuses to start it; unstated is a warning, because every module written before
the field existed omits one and refusing would empty the library on upgrade. The field stays null
rather than being filled in with whatever is loaded — exporting a module should not put words in its
author's mouth.

The failure this prevents is the bad kind: a `.mship` written for Mothership, loaded into a system
with different stats, produces a playable-looking session where every check reads `undefined`, no
error anywhere, and the first sign of trouble is a roll target of `NaN`.

**There is still no second ruleset, and the reason has changed.** At 2.20.0 it was "a plugin
interface with one implementation is a rename". Now the seam is finished, so the honest statement is
that writing a second system is a game-design job rather than an engineering one, and inventing a
role-playing game to prove a refactor is not something this project should do. The two ruleset test
files define a deliberately different system — three stats, no classes, no panic, four creation
steps — and assert the engine runs it. They are the specification for anybody who wants to write a
real one.

**What is left, and it is smaller.** `gear.js` is Mothership's standard kit and `defineModule` merges
it into every module, so a non-Mothership system inherits pulse rifles unless every module overrides
them. A ruleset should supply its own. That is the next piece and it is about the size of the
`rules.js` split.

### Where the time went

2.20.0's analytics could not answer "which rooms stall" and said so at length: the feed stamps every
line with the clock but not with the room, so deriving it would have meant attributing lines by
inference — wrong exactly when the party is split.

It is recorded now rather than derived. `commitW` is the single choke point every world write goes
through, so when the party's centre of mass moves, the minutes since it last moved are banked
against the room being left. One write, one place, and it cannot disagree with itself.

**It is the party's time and the report says so.** `w.room` is `majorityRoom`, so while the crew is
split this counts everyone as being wherever most of them were standing. That is a real limitation,
it is stated on the panel rather than buried, and a session that never splits has no error in it.

Two details worth knowing. The room a session ENDS in has never been left, so its span is added at
read time — without that the last room of every evening reads as zero minutes, which is wrong in a
way that looks like a finding. And time is averaged over the evenings that reached a room rather
than over all of them: a room visited once and dwelt in for an hour is not a twenty-minute room
because two other sessions missed it.

`stalled` marks a room the party sat in for half an hour without touching anything. **It is not a
verdict.** Sometimes that is a puzzle nobody could solve and sometimes it is a conversation that
went well. The threshold is arbitrary, which is why it is a named constant rather than a literal
buried in a comparison.

There is a test that plays an actual session and walks the crew around. The arithmetic was never the
fragile part — whether `commitW` is still the choke point every room change goes through is, and
that is the assertion that would fail if a tenth mover ever wrote `wRef.current.room` directly.
