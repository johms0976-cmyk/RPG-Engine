...
- **In the browser.** "Write a module" on the shelf. Rooms, exits and endings as forms, everything
  deeper as JSON, and the engine's own validation running on every keystroke — the same check the
  library card makes, so what it says while you type is what the shelf will say. Export a `.mship`
  file or put it straight on the shelf and play it. No build step and no terminal.
...
Every module also has a **Print it** button on its library card: the rooms, the cast, the threats,
the handouts, the endings and as many blank character sheets as it suggests, laid out for a printer.
A world is optional, so the same pack works for prepping on a train and for reprinting in week
three with what the table has already used struck through.

The editor cannot open the two modules that ship with this engine, and that is the format rather
than a limitation of the editor: both carry `hooks`, which is JavaScript, and a portable module is
data and carries no code. That is also why it is safe to load somebody else's.
...
**Now** — a front door, a hosted demo, modules you can write and load without a build step, remote
play over direct connections (see [`docs/REMOTE_PLAY.md`](docs/REMOTE_PLAY.md)), a Warden deck that
fits on a phone, the whole module printable from a button, and a report on what your modules
actually did at a table.

**Next** — finishing the rulesets work. The seam is in
([`docs/RULESETS.md`](docs/RULESETS.md)) and Mothership now loads through it, but character
creation, panic and module loadouts are still Mothership-shaped, and that document says exactly
which three pieces are left.

**Later** — solo play driven by the oracle, and time-in-room instrumentation so the analytics can
answer which rooms stall rather than only which rooms nobody reached.
...
src/screens/   Warden deck, play, creator, library, module editor
