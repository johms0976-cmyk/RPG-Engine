...
**Next** — a second ruleset, which is a game-design job rather than an engineering one. The seam is
finished ([`docs/RULESETS.md`](docs/RULESETS.md)): creation, panic and module compatibility are all
ruleset-driven, and a system with three stats and no classes runs end to end. What no ruleset can
supply yet is its own standard equipment — `gear.js` is still Mothership's kit.

**Later** — solo play driven by the oracle.
