...
describe("ANOTHER BUG HUNT", () => {
  it("starts at the landing zone, with the storm NOT yet running", () => {
    expect(mod.start).toBe("lz");
    expect(mod.rooms.lz).toBeTruthy();
    /* The ten hours belong to Heron. Scenario one must not spend
       them — see index.js `onStart` and `beginStorm` in sim.js. */
    expect(mod.onStart.find((e) => e.countdown)).toBeUndefined();
  });

  it("starts the storm on arrival at the hangar, once, at 600 minutes", () => {
    const said = [];
    const flags = {};
    const api = {
      world: () => ({ flags }),
      flag: (k, v) => Object.assign(flags, typeof k === "string" ? { [k]: v ?? true } : k),
      effects: (es) => said.push(...es),
      say: () => {},
    };
    mod.hooks.beginStorm(api);
    const cd = said.find((e) => e.countdown);
    expect(cd.countdown.minutes).toBe(600);
    expect(cd.countdown.onZero[0].run).toBe("stormBreaks");
    expect(flags.storm_started).toBe(true);

    /* Second entry to the hangar must not restart it. */
    said.length = 0;
    mod.hooks.beginStorm(api);
    expect(said).toHaveLength(0);
  });

  it("spends a charge of acid on every throw, hit or miss", () => {
    const acid = mod.threats.carc.counters.find((c) => c.id === "acid");
    expect(acid.onBreak.some((e) => e.run === "spendAcid")).toBe(true);
    expect(acid.onHold.some((e) => e.run === "spendAcid")).toBe(true);
  });

  it("puts the noble in the Court, where the assimilate hook can reach it", () => {
    const placed = mod.rooms.court.onFirstEnter
      .filter((e) => e.threat).map((e) => e.threat.id);
    expect(placed).toContain("noble");
    expect(mod.threats.noble.ambushes).toBe(false);
  });
...
describe("ANOTHER BUG HUNT — nothing on the mantelpiece stays unfired", () => {
  ...
  it("will not offer a faction's plan until that faction lead has been met", () => {
    const ids = ["mission_leave", "mission_study", "mission_hog"];
    const leads = { mission_leave: "brookman", mission_study: "edem", mission_hog: "valdez" };
    for (const id of ids) {
      const a = mod.actions.find((x) => x.id === id);
      expect(a.when).toContain(`met:${leads[id]}`);
    }
  });

  it("puts the first Shriek stage inside the storm", () => {
    expect(mod.tracks.shriek.stages[0].after).toBe("1d10*60");
  });
});
