/* ============================================================
   ...
   All three of those pieces landed in 2.21.0:

     1. `create.steps` declares which parts of character creation
        a system has. `Creator.jsx` and `CreatorPhone.jsx` render
        the declared ones and nothing else, so a system with no
        classes gets a creator with no class picker rather than
        a broken one.
     2. `panic.table` empty means the system has no panic
        mechanic, and `queuePanic` becomes a no-op rather than
        rolling against an empty table. The one class ability
        that touches panic is found by its `ability` key instead
        of by the class being called "teamster".
     3. `defineModule` takes a `ruleset` field and refuses a
        module written for a system that is not loaded.
...
/** The parts of character creation this engine knows how to render.
    A ruleset picks from these; it cannot invent a new one, because a
    step nothing knows how to draw is a step that silently does
    nothing. Adding one means adding it here AND to both creators. */
export const CREATE_STEPS = ["name", "stats", "class", "skills", "loadout", "flavour"];
...
export function defineRuleset(raw) {
  ...
  /* CREATION, AS A LIST OF STEPS RATHER THAN A FLOW.

     Not a form engine. The screens still know how to roll stats and
     how to spend skill points; what they no longer assume is that
     every system HAS those things. A ruleset says which steps exist
     and the creator renders exactly those, so a classless system
     gets a creator with no class picker rather than one that renders
     an empty list and refuses to let anybody finish. */
  const create = r.create || {};
  const steps = Array.isArray(create.steps) ? create.steps : [...CREATE_STEPS];
  for (const step of steps) {
    if (!CREATE_STEPS.includes(step)) problems.push(`unknown creation step "${step}"`);
  }
  if (steps.includes("class") && Object.keys(r.classes || {}).length === 0) {
    problems.push("creation has a class step but the ruleset declares no classes");
  }
  if (steps.includes("skills") && Object.keys((r.skills || {}).tree || {}).length === 0) {
    problems.push("creation has a skills step but the ruleset declares no skill tree");
  }
  ...
  return {
    ...
    /* An empty table means the system has no panic mechanic, and
       every call site treats that as "do nothing" rather than as a
       missing lookup. `enabled` is derived rather than declared so
       the two cannot disagree. */
    panic: {
      table: (r.panic && r.panic.table) || [],
      triggers: (r.panic && r.panic.triggers) || {},
      enabled: !!((r.panic && r.panic.table) || []).length,
    },
    wake: r.wake || [],
    ...
    create: {
      steps,
      /* What a stat roll looks like on the sheet, for the creator's
         "re-roll" button to explain itself with. Prose the ruleset
         wrote, never generated here. */
      statNote: create.statNote || "",
    },
