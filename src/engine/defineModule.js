...
import { coverageNotes } from "./coverage.js";
...
function validate(raw, items) {
  ...
  Object.entries(threats).forEach(([id, t]) => {
    ...
    /* A misspelled tactic would silently fall back to "weakest" and the
       author would never learn their creature was not behaving as
       written — the worst class of bug, because it looks like a
       design disagreement rather than a typo. */
    if (t.tactics != null && !TACTICS.includes(t.tactics)) {
      problems.push(`threat "${id}" has unknown tactics "${t.tactics}" — one of: ${TACTICS.join(", ")}`);
    }
    if (t.morale != null && (typeof t.morale !== "number" || t.morale < 0 || t.morale > 1)) {
      problems.push(`threat "${id}" morale must be a fraction of health between 0 and 1`);
    }
  });
...
export function defineModule(raw) {
  ...
  /* ============================================================
     WHICH SYSTEM WAS THIS WRITTEN FOR?

     Every module in this repository declares `loadouts` in
     Mothership's shape, names Mothership skills in its features and
     gives its threats a Combat score. None of them ever said so,
     because until 2.20.0 there was nothing else they could have been
     written for.

     Now there is, and the failure this prevents is a bad one and a
     silent one: load a `.mship` file written for Mothership into a
     system with different stats and you get a playable-looking
     session where every check reads `undefined`, no error anywhere,
     and the first sign of trouble is a roll target of NaN.

     UNSTATED IS A WARNING, NOT A PROBLEM. Every module written
     before this field exists, including the four in this repository,
     omits it — treating that as a refusal would empty the library on
     upgrade. Stated and wrong is a problem, and the library card
     already refuses to start anything with problems.
     ============================================================ */
  const rs = activeRuleset();
  const wantsRuleset = typeof raw.ruleset === "string" ? raw.ruleset : null;
  if (wantsRuleset && rs && wantsRuleset !== rs.id) {
    problems.push(
      `written for the "${wantsRuleset}" ruleset, but "${rs.id}" is loaded. `
      + "Its loadouts, skills and stats will not line up.");
  } else if (!wantsRuleset) {
    warnings.push(
      "does not say which ruleset it was written for. Assuming the one that is loaded — "
      + "add `ruleset: \"" + (rs ? rs.id : "mothership1e") + "\"` to be sure.");
  }

  const mod = {
    engine: raw.engine || "^2.0.0",
    /* Null rather than defaulted to the loaded one. A module that did
       not say must keep not having said, or exporting it would put
       words in its author's mouth. */
    ruleset: wantsRuleset,
    id: raw.id,
    title: raw.title,
  ...
  /* THE COVERAGE REPORT. Not correctness — see engine/coverage.js.
     `problems` says this cannot work and `warnings` says this will
     silently do nothing; neither answers "is this finished". It is
     computed after `mod` is assembled because it reads the merged
     result (auto-directors, defaulted maps) rather than the raw
     input, and it is deliberately NOT pushed into `warnings`:
     a warning claims something is wrong, and none of this is. */
  try {
    mod.coverage = coverageNotes(mod);
  } catch {
    /* A report that throws takes the app down at import time,
       which is precisely backwards — INV-11. No report is a fine
       outcome; no library is not. */
    mod.coverage = [];
  }
...
export const moduleCard = (m) => ({
  ...
  problems: m.problems, warnings: m.warnings, coverage: m.coverage || [],
