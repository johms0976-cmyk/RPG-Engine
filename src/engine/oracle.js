...
export function parseCommand(text, ctx) {
  ...
  /* no verb - try to infer from the noun alone */
...
export function npcReply(npc, said, state, rng, memory) {
  ...
  knows.forEach((k, i) => {
    ...
    const kw = contentWords(tokenise(k));
    let score = 0;
    for (const w of words) {
      if (w.length < 4) continue;
      if (kw.some((t) => t === w || t.startsWith(w.slice(0, 4)))) score += 1;
    }
    // a small bias toward the earlier, more important facts
    score += (knows.length - i) * 0.05;
