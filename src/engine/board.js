...
export const MARK_KINDS = {
  ...
  /* Two kinds the *world* writes rather than the crew. They exist so
     Ypsilon's water and sound can be drawn on the shared map instead
     of being state only the simulation could see — see §10.1 and
     §10.2. A module that never sets them never shows them. */
  wet: { label: "Water", glyph: "≈", blurb: "Standing water. It will not cross this willingly.", sim: true },
  loud: { label: "Noise", glyph: "))", blurb: "Something in here is making a lot of sound.", sim: true },
};
...
/* A GROUP OF MARKS THE SIMULATION OWNS.

   Player marks are written once and rubbed out by hand. Some marks
   are not like that: Ypsilon's wet rooms carry a countdown, so the
   label changes every simulation step and the honest operation is
   "these are the water marks now", not "add one, find and remove
   the stale one, hope nothing raced".

   Grouped marks are namespaced by `owner`, replaced wholesale, and
   carry `sim: true` so the board can decline to let a player rub
   out something the world is asserting. Nothing here touches marks
   the crew drew — that is the entire reason for the namespace. */
export const setGroupMarks = (marks, owner, list, clock = 0) => [
  ...(marks || []).filter((m) => m.owner !== owner),
  ...(list || []).map((m, i) => ({
    id: `${owner}:${m.room}:${i}`,
    room: m.room,
    kind: MARK_KINDS[m.kind] ? m.kind : "note",
    text: String(m.label || m.text || "").slice(0, 60),
    by: null,
    byName: null,
    owner,
    sim: true,
...
export function canRemoveMark(mark, { pcId, isWarden }) {
  /* Nobody rubs out the world. A wet room is wet whether or not the
     crew would prefer it were not, and a mark the simulation rewrites
     every ten minutes would come straight back anyway — which reads
     as a broken button rather than as a rule. */
  if (mark && mark.sim) return false;
...
const PRIORITY = ["danger", "wet", "loud", "locked", "loot", "safe", "note"];
