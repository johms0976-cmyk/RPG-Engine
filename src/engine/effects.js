 ...
 *           dead:it   met:brookman   clock>90   meter:bends>=5   crew:marine
 *           !anything
 */
export function test(cond, ctx) {
  ...
  return String(cond).split(" and ").every((clause) => {
    ...
    switch (key) {
      ...
      /* Has this crew actually spoken to them? `met` has been on
         every NPC's state since world.js was written and is set by
         both the talk path and the approach path, and nothing could
         ask about it — so a module that wanted "you may only take
         Brookman's plan once Brookman has told you what it is" had
         to invent a flag and remember to raise it in every branch.
         Deliberately not the same as `npc:` (alive and untaken) or
         `here:` (in the room right now): this one is about the
         table's own history. */
      case "met": return !!(w.npcs[arg] && w.npcs[arg].met);
...
export function runEffects(effects, api, vars = {}) {
  ...
  for (const raw of list) {
    ...
    if (e.pick) {
      /* The module's own weighted branch, off the table's seeded
         stream when the api carries one. `api.rng` is the same
         generator the atmosphere uses, so a save replays the branch
         it actually took rather than a fresh one. */
