 ...
 *
 * PSG 12.4, and the weapon chart's "Shots: 1(3)" / "1(5)" notation:
 * an untrained shooter empties a fully automatic weapon every time
 * they fire it and must reload; a trained one has the trigger
 * discipline to get `shots` bursts out of the magazine.
 *
 * The previous form read `burst ?? 3` against the magazine, and both
 * automatic weapons in gear.js carry `burst === shots` (SMG 5/5,
 * Pulse Rifle 3/3) — so trained and untrained spent exactly the same
 * ammunition and Firearms / Military Training bought nothing at all
 * on the two weapons the rule exists for.
 */
export function shotsForAttack(pc, weapon) {
  if (!weapon.shots) return 0;
  if (!weapon.auto) return 1;
  return isTrainedShooter(pc) ? 1 : weapon.shots;
}
