...
import { floorVerdict } from "./floor.js";
...
/** Everyone still standing, in crew order. Deliberately not sorted
    ...
    `compare` is the one exception, and it is opt-in. A table that
    has turned the floor ledger on passes `starvationOrder` here, so
    the round opens with whoever has had least of it rather than
    with the same person every time. The default stays crew order —
    the argument above is still right for a table that has not asked
    for anything else, and an invisible reorder nobody requested
    would be a worse surprise than a visible one. */
export function makeScene(crew, label, compare = null) {
  let order = (crew || [])
    .filter((c) => c.alive !== false && !c.unconscious)
    .map((c) => c.id);
  if (compare) order = [...order].sort(compare);
...
export const TEMPO_FREE = new Set([
  ...
  "readHandout",
  /* Deciding to tell the table what you saw is speech, not a turn.
     A brake that held it would be the software deciding when
     somebody may speak, which is a different and much worse product
     than the one this file is for. */
  "shareSecret",
  /* A reaction is a body in a chair, not a turn, and a brake that
     held one would be the software deciding when somebody may
     wince. Offering to help is speech for the same reason. */
  "react", "offerAssist", "withdrawAssist",
]);
...
export function tempoVerdict({ w, action, pcId, now = Date.now(), lastActed = {}, crew = null }) {
  ...
  /* THE WEAKEST BRAKE, DELIBERATELY LAST.

     Everything above is something a person asked for: the Warden
     held the table, the Warden opened a round, the table agreed a
     rate limit. The floor is the only brake nobody pressed, so it
     is the only one that could be experienced as the software
     having an opinion — and it goes last so that it can never be
     the reason a player is held when a stronger, explained brake
     would have said something truer.

     It is inert without `crew`, and inert unless `w.floor.on`. */
  if (crew) {
    const floor = floorVerdict({ w, pcId, crew, now });
...
export const WAIT_TEXT = {
  ...
  /* Never says who. See rule 1 in engine/floor.js — the whole
     subsystem is worthless the moment it starts naming people. */
  floor: "The room is waiting on someone else.",
};
