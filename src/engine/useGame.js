...
import {
  ...
  makeMark, addMark, dropMark, canRemoveMark,
...
export function useGame(mod, settings = {}) {
  ...
  }, [say, sayIn, here, mod.threats, setThreat, rng]);
  ...
  apiRef.current = useMemo(() => ({
    ...
    countdown, stopCountdown, offerRest, rollNow, ask, startCombat, endGame, doMove, describeRoom, commitW, commitCrew]);
  ...
  const wardenBreather = useCallback((on) => {
    const t = tempoOf(W());
    const next = on == null ? !t.breather : !!on;
    patchTempo({ breather: next ? { since: Date.now(), clock: W().clock } : null });
  ...
  const wardenScene = useCallback((op, arg) => {
    ...
    if (op === "start") {
      const scene = makeScene(crew, arg || null);
  ...
  const warden = useMemo(() => ({
    ...
    wardenClearRecap, wardenRate, wardenInitiative, wardenNudge, wardenShowTo,
