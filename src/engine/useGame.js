...
import {
  armorSave, bestArmorItem, baseValue, clampTarget, panicEffect, WAKE_TABLE, CLASSES, RULESET,
...
export function useGame(mod, settings = {}) {
  ...
  /* ============================================================
     ...
     Every write to the world goes through here, which makes this the
     one place a room change can be seen without instrumenting the
     nine callers that cause one. When `room` moves, the minutes since
     it last moved are banked against the room being left.

     WHAT THIS MEASURES, PRECISELY. `w.room` is the party's centre of
     mass — `majorityRoom` — not any one character's position. So this
     is "how long the group was in each room", and while the party is
     split it attributes the time to wherever most of them were
     standing. That is a real limitation and it is the reason 2.20.0
     shipped without this: a number that is quietly wrong under a
     condition nobody checks is worse than no number.

     It is here now because the alternative considered then — deriving
     it from the feed — was wrong in the same case AND unfixable,
     whereas this is merely coarse and says so. `analytics.js` labels
     it as the party's time rather than as each player's.
     ============================================================ */
  const commitW = useCallback((patch) => {
    const cur = wRef.current;
    if (patch.room && patch.room !== cur.room) {
      const clock = patch.clock ?? cur.clock ?? 0;
      const since = cur.roomSince ?? clock;
      const spent = Math.max(0, clock - since);
      patch = {
        ...patch,
        roomTime: { ...(cur.roomTime || {}), [cur.room]: ((cur.roomTime || {})[cur.room] || 0) + spent },
        roomSince: clock,
      };
    }
    wRef.current = { ...cur, ...patch };
    setW(wRef.current);
  }, []);
  ...
  const rollNow = useCallback((req) => {
    ...
    setLastRoll({ ...r, label, who: pc.name, breakdown: m.breakdown });
      ...
      /* WHAT IT WAS FOR, not just what was rolled. `label` is the stat
         — "Strength (Athletics)" — which is enough to reread an
         evening and not enough to tell two doors apart. `tags` and
         `why` are already on the request and were already in the feed
         line; they were simply never kept anywhere a report could
         reach. `engine/analytics.js` groups door rolls by `why` to
         find the lock a table spent forty minutes on. */
      tags: req.tags || [],
      why: req.why || null,
      declared: !!r.declared });
  ...
  const queuePanic = useCallback((pcId, reason) => {
    /* A SYSTEM WITH NO PANIC TABLE HAS NO PANIC.

       Seventeen call sites reach this — a critical failure, a big
       hit, a crew death, first contact — and none of them should
       have to know whether the loaded ruleset has the mechanic.
       Refused here, once, so every trigger keeps firing harmlessly
       under a system that does not have it. The alternative was
       seventeen guards, sixteen of which somebody eventually
       forgets. */
    if (!RULESET.panic.enabled) return;
  ...
  const doPanic = useCallback((pcId, depth = 0) => {
    ...
    let effRoll = dN(2, 10);
    /* Found by the ABILITY the class declares, not by the class being
       called "teamster". The class data has carried `ability:
       "panicReroll"` since 2.0 and nothing read it; a ruleset whose
       equivalent class has another name was getting no reroll. */
    const myClass = CLASSES[pc.cls];
    const hasReroll = !!(myClass && myClass.ability === "panicReroll");
    if (hasReroll && !pc.usedPanicReroll && effRoll + pc.stress - pc.resolve >= 14) {
      const second = dN(2, 10);
      say("panic", `${pc.name} is a ${myClass.name} and has seen worse. Re-rolling ${effRoll} → ${second}.`,
  ...
  const doFreeAction = useCallback((text) => {
    ...
    switch (intent.kind) {
      ...
      default: {
        ...
        /* THE SENTENCE THE MODULE HAD NO ANSWER FOR.

           Carried on the marker line rather than echoed into the
           feed, because the table should not see their own words
           read back at them — that is a transcript for an AUTHOR,
           not a line of play. `engine/misses.js` reads these out
           of a finished feed and tallies them, which is how a
           module learns what listeners it is missing from the
           people who actually tried things. */
        sayIn(where, "system", `[oracle · ${odds} · rolled ${o.roll} vs ${o.chance}]`,
          { miss: String(text || "").trim() });
        return;
      }
    }
