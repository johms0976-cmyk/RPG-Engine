...
export function useGame(mod, settings = {}) {
  ...
  const ask = useCallback((req) => {
    ...
    setPending({ kind: "roll", req: { kind, name, skill, mode, tags: req.tags, reason: req.reason, effects: req.effects, pcId: req.pcId } });
  }, []);
  ...
  const vanish = useCallback((opts = {}) => {
    ...
    if (opts.stress) stressCrew(evalDice(opts.stress), "another one gone");
  ...
  const advance = useCallback((mins) => {
    ...
    expireBuffs(clock);
    tickTracks(clock);
  }, [commitW, say, mod.clocks, wakeSleepers, expireBuffs]);
  ...
  }, [mod.threats, ctxFor, say, stress, hurt, rollNow, queuePanic, commitCombat]);
  ...
  const attackWith = useCallback((weaponId, targetUid, assistId) => {
    ...
    afterAction(1);
  }, [items, houseRules, ctxFor, say, noise, patchPc, mod.threats, queuePanic, commitCombat, afterAction, finishCombat, awardXp, setThreat, logRoll]);
  ...
  }, [mod.threats, setThreat, say, startCombat, rng]);
  ...
  apiRef.current = useMemo(() => ({
    ...
    advance, noise, vanish, rollTable, run, setThreat, startTrack, awardXp,
  ...
  }), [mod, items, houseRules, say, flag, give, take, stress, stressCrew, meter, heal, hurt, panic,
    addCondition, addBuff, advance, noise, vanish, rollTable, run, setThreat, startTrack, awardXp,
  ...
  return {
    ...
    attackWith, reloadWeapon, aim, combatMove, setTarget, useCounter, fleeCombat, endPcTurn,
