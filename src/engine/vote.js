...
export const VOTE_TOPICS = {
  ...
  /* THE FLOOR, PUT TO THE TABLE.

     `floor.js` is off by default on the argument that four friends
     who have played together for a decade will resent it — which is
     right, and which assumed a Warden sitting there to turn it on
     when the table is six people and two of them have not spoken in
     forty minutes. Its only switch was in `TempoTab`, inside
     `WardenDeck`, which wardenless mode locks away by design. So the
     one configuration with nobody watching the airtime was the one
     configuration that could not ask for the airtime to be watched.

     A vote rather than a button, because rule 6 in `floor.js` says
     everything stays inert until somebody asks for it — and a table
     asking itself is the most literal possible reading of that. It
     also keeps the thing that makes the floor tolerable: it arrives
     because the room agreed to it, not because software decided the
     room had a problem. */
  floor: {
    label: "Watch who hasn't spoken?",
    blurb: "The room keeps an eye on anyone getting talked over, and holds the fast hands for a beat. Nobody is ever named.",
    options: [
      { id: "yes", label: "Keep an eye" },
      { id: "no", label: "Leave it" },
    ],
    fallback: "no",
  },
  ...
  /* THE RULING, WHICH IS THE ONE THE EMPTY CHAIR COULD NOT MAKE.

     `director.js` says it plainly in its own header: it is not an
     improviser, and when a player has an idea the module never
     anticipated the oracle answers and the fiction does not update.
     That is the largest single gap in wardenless play and it is not
     a gap software can close by getting cleverer — somebody has to
     decide what is true, and there is nobody in the chair.

     So the table decides. A player proposes the sentence, it goes
     on the ballot as the vote's `note`, and if it carries it is
     committed through the same `makeRuling({ by: "table" })` path a
     Warden's would be. From that point the room describes it, the
     parser matches it, and the transcript records who made it up.

     The fallback is `no`, and that asymmetry is deliberate. Every
     other topic here asks the table to change something about how
     they are playing; this one asks them to change what is true.
     Silence should never invent a fact — a proposal nobody answers
     is a proposal that failed, and the player is free to ask again
     with a smaller one.

     THE PROPOSER DOES NOT GET A VETO AND DOES NOT GET A BONUS.
     They vote like everybody else, from the same denominator. A
     table where the loudest player can carry their own ideas is
     the failure `floor.js` exists to correct, arriving by a
     different door. */
  ruling: {
    label: "Is that true?",
    blurb: "Somebody wants to add something the module never said. If it carries, the room will remember it.",
    options: [
      { id: "yes", label: "That's true" },
      { id: "no", label: "Not that" },
    ],
    fallback: "no",
  },
};
