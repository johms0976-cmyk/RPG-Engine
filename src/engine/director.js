...
import { mergeListeners } from "./listenerPack.js";
...
/* ============================================================
   THE EMPTY CHAIR'S HAND ON THE CLOCK

   All three named, because all three are social judgements about
   how big a "later" a table will accept without feeling robbed,
   and a table will want to disagree with every one of them.
   ============================================================ */

/** Don't bother for less than this. A five-minute skip is not worth
    a line of feed, and a director that announces trivia teaches the
    table to stop reading it. */
export const PASS_TIME_MIN = 10;

/** The largest single skip. Twenty minutes is about the biggest
    "later" a Warden declares without asking the table first — past
    that it is a scene transition, which is a judgement call and
    therefore `vote.js`'s business, not this rung's. */
export const PASS_TIME_MAX = 20;

/** And how long between skips. Deliberately long: the clock turning
    twice inside five minutes reads as the software being impatient
    with you, which is the exact failure `pacing.js` opens by
    promising to avoid. */
export const PASS_TIME_GAP_MS = 6 * 60 * 1000;
...
export function safeMove(move, { w, mod, crew } = {}) {
  ...
  /* THE SIXTH CHECK — spending the table's evening.

     `rungPacing` bounds this already, but the bound belongs here
     too: `safeMove` is the single place every Move passes through,
     and a rung is one refactor away from forgetting its own cap.
     A negative or absent skip is a bug that would run the clock
     backwards; an unbounded one is a rung that could hand a table
     an ending they never played towards. */
  if (move.kind === "passTime") {
    const mins = Number(move.mins);
    if (!Number.isFinite(mins) || mins < PASS_TIME_MIN || mins > PASS_TIME_MAX) return null;
  }
...
export function rungPacing({ mod, w, now, startedAt, combat, pending, lastPassAt = 0 }) {
  ...
  /* ------------------------------------------------------------
     SPENDING TIME, NOT REPORTING IT.

     `w.clock` only ever moved inside a player action, so a table
     that spent fifteen real minutes arguing in a corridor cost the
     fiction nothing: `onTick` never fired, countdowns never fell,
     and the thing in the vents stayed exactly where it was. With a
     Warden that is fine, because "right, ten minutes go by" is a
     sentence a person says roughly once a scene. With the chair
     empty nobody was saying it, and `warden.passTime` — the button
     that does exactly this — lives on a deck this mode deliberately
     cannot reach.

     So the drifting verdict now turns the clock rather than reading
     it out. This rung had every number it needed to do that and was
     announcing a countdown instead.

     WHY THIS IS SAFE TO TAKE WITHOUT A PERSON:

       · It composes nothing. `passTime` emits the engine's own
         neutral marker — "— 10 minutes pass —" — so INV-1 holds
         without an author writing a line.
       · It only ever moves the clock FORWARD, and only by the
         deficit already measured. It cannot skip a countdown past
         zero that the module wanted narrated, because `advanceNow`
         runs the module's own `onTick` across the interval exactly
         as a player action would.
       · Three refusals below, and all of them are the ones a
         person would observe by instinct.
     ------------------------------------------------------------ */

  /* Never inside a fight — rounds are seconds and a fight already
     owns the clock. Never while a prompt is sitting on somebody's
     screen, because time passing while they decide is punishing
     them for reading. And never twice in quick succession. */
  if (combat) return null;
  if (pending) return null;
  if (now - lastPassAt < PASS_TIME_GAP_MS) return null;

  /* How far behind the fiction has fallen, capped. `pacingOf`
     reports the expectation; the deficit is what a Warden is
     eyeballing when they decide how big a "later" to declare. */
  const realMins = Math.max(0, (now - startedAt) / 60000);
  const expected = realMins * rate;
  const deficit = Math.round(expected - (w.clock || 0));
  if (deficit < PASS_TIME_MIN) return null;

  const mins = Math.min(deficit, PASS_TIME_MAX);
  const soon = soonestClock(w);
  return {
    kind: "passTime",
    rung: "pacing",
    mins,
    /* Carried so the executor can follow the skip with the clock
       that is now closest — which is what the old `clock` Move did
       on its own, and is worth keeping as the second half of this
       beat rather than as the whole of it. */
    countdown: soon ? soon.label : null,
    left: soon ? Math.max(0, soon.left - mins) : null,
  };
}
...
export function directorPlan({
  ...
  /* When the director last spent fiction-minutes on the table's
     behalf. See `rungPacing` and PASS_TIME_GAP_MS — the empty
     chair's only hand on the clock, and the one thing on this
     ladder that changes the world without anybody acting. */
  lastPassAt = 0,
  ...
  const args = {
    ...
    heard, sessionEndsAt, lastCallAt, lastPassAt,
  };
...
export function rungListen({ mod, w, crew, feed = [], now, lastMoveAt, heard = {} }) {
  const d = (mod && mod.director) || null;
  /* THE PACK. Module listeners first, then the common ones — see
     engine/listenerPack.js for why a generic listener may observe
     and may never describe. A module with no listeners at all now
     gets the common set rather than nothing, which is the point:
     Dead Weight authored eight and the empty chair was silent for
     every other thing a table says. */
  const listeners = mergeListeners(d);
...
export const MOVE_HARSH = {
  ...
  nudge: false,
  npcSay: false,
  /* Time passing is not cruelty. It is the thing a Warden does to
     get a stalled table moving, and counting it as harsh would let
     a drifting evening trip the breather rung — which would answer
     "nothing is happening" with "let us all stop for five minutes". */
  passTime: false,
  startScene: false,
  breather: false,
  callback: false,
...
export const isSpoken = (move) => !!(move && (move.speaks || move.kind === "describe"
  ...
  || move.kind === "breather"
  /* Spending the table's evening is loud by definition — the clock
     is the module's main antagonist — so a Warden must see it in
     the strip before it lands. */
  || move.kind === "passTime"));
  ...
  switch (move.kind) {
    ...
    case "passTime":
      return move.countdown
        ? `Let ${move.mins} minutes pass — ${move.countdown} then ${move.left}m`
        : `Let ${move.mins} minutes pass`;
