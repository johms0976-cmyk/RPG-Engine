...
export const LADDER = [
  ...
  "attack", "roll", "npc", "floor", "breather", "pressure", "pacing",
  "callback", "atmosphere", "silence",
];
...
export function safeMove(move, { w, mod, crew } = {}) {
  ...
  if (move.kind === "combat") {
    ...
    if (t.loc && w && t.loc !== w.room) return null;
 ...
 */
export function rungAttack({ mod, w, crew }) {
  const d = (mod && mod.director) || null;
  ...
  for (const a of d.attacks) {
    ...
    if (a.room && a.room !== w.room) continue;
    ...
    return {
      ...
      room: w.room, surprise: !!a.surprise,
 ...
 *     it is a transcript.
 */
export function rungCallback({ w, now, lastLineAt, lastCallbackAt = 0, calledBack = {} }) {
  ...
  return {
    ...
    text: clue.text, room: w.room, speaks: true,
  };
}
...
export function directorPlan({
  ...
  /* `rungLastCall` sits high because it is a fact about the room
     ...
     said should beat filling a silence — but never beat safety,
     a pending prompt, or a fight already in progress. */
  const rungs = [rungSafety, rungPending, rungCombat, rungAftermath, rungEnding,
    rungLastCall, rungScripted, rungListen, rungAttack, rungCallRoll, rungNpc, rungFloor,
    rungBreather, rungPressure, rungPacing, rungCallback, rungAtmosphere];
