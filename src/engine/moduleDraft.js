...
import { activeRulesetId } from "./ruleset.js";
...
export function blankDraft({ title = "UNTITLED", id } = {}) {
  ...
  return {
    engine: "^2.0.0",
    /* Stated by default in anything written here, because the editor
       DOES know which system it is running under and a module that
       says so cannot later be loaded into one where its loadouts
       resolve to nothing. See docs/RULESETS.md. */
    ruleset: activeRulesetId(),
    id: id || slug(title),
    title,
    blurb: "",
    byline: "",
