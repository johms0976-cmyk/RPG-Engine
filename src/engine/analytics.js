/* ============================================================
   ...
   ------------------------------------------------------------
   TIME IN A ROOM, AND WHAT IT IS NOT

   2.20.0 shipped without this and said why: the feed stamps every
   line with the clock but not with the room, so deriving it would
   have meant attributing lines to rooms by inference, which is
   wrong exactly when the party is split.

   2.21.0 records it instead of deriving it. `commitW` banks the
   minutes against the room being left whenever the party's centre
   of mass moves, which is one write at one choke point and cannot
   disagree with itself.

   IT IS THE PARTY'S TIME, NOT EACH PLAYER'S. `w.room` is
   `majorityRoom` — where most of the crew is standing — so while
   the party is split this attributes the whole clock to whichever
   room held the majority, and the two people down the vent are
   counted as being where they are not. That is a real limitation.
   It is reported as "the party spent" rather than as "players
   spent" so the number cannot be read as something it is not, and
   a session that never split has no error in it at all.

   `stalled` is deliberately NOT a verdict. It marks a room the
   party spent a long time in without touching much, which is
   sometimes a puzzle nobody could solve and sometimes a
   conversation that went well. The report says which; the author
...
/** Long enough that a Warden would remember the room as having taken
    a while. Arbitrary, and named rather than buried so it can be
    argued with. */
...
export function sessionReport(mod, w, feed = []) {
  ...
  const roomTime = (w && w.roomTime) || {};
  /* The room the session ENDED in has never been left, so its time was
     never banked. Without this the last room of every evening reads as
     zero minutes, which is both wrong and wrong in a way that looks
     like a finding. */
  const live = { ...roomTime };
  if (w && w.room) {
    const open = Math.max(0, (w.clock || 0) - (w.roomSince || 0));
    live[w.room] = (live[w.room] || 0) + open;
  }
  ...
  const rooms = Object.entries(mod.rooms || {}).map(([id, r]) => {
    ...
    return {
      ...
      /* Minutes the PARTY spent here. See the header for what that
         does and does not mean when the crew splits up. */
      minutes,
      ...
      /* Half an hour is the threshold and it is arbitrary, which is
         why it is a named constant rather than a literal buried in a
         comparison. It is roughly the point at which a Warden
         remembers a room as having taken a while. */
      stalled: minutes >= STALL_MINUTES && touched.length === 0 && features.length > 0,
    };
  });
...
export function sessionDigest(report) {
  ...
  return {
    ...
    rooms: report.rooms.map((r) => ({
      ...
      minutes: r.minutes, stalled: r.stalled,
    })),
...
export function tableReport(mod, digests = []) {
  ...
  for (const d of mine) {
    ...
    for (const r of d.rooms || []) {
      const e = roomSeen.get(r.id) || { reached: 0, touchedIn: 0, minutes: 0, stalled: 0 };
      if (r.reached) e.reached += 1;
      if (r.touched > 0) e.touchedIn += 1;
      e.minutes += r.minutes || 0;
      if (r.stalled) e.stalled += 1;
      roomSeen.set(r.id, e);
    }
  ...
  return {
    ...
    rooms: Object.entries(mod.rooms || {}).map(([id, r]) => {
      const e = roomSeen.get(id) || { reached: 0, touchedIn: 0, minutes: 0, stalled: 0 };
      return {
        id, name: r.name || id,
        reached: e.reached,
        touchedIn: e.touchedIn,
        minutes: e.minutes,
        /* Averaged over the evenings that got there, not over all of
           them — a room reached once and dwelt in for an hour is not
           a twenty-minute room because two other sessions missed it. */
        perVisit: e.reached ? Math.round(e.minutes / e.reached) : 0,
        stalled: e.stalled,
