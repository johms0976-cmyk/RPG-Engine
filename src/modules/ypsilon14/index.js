...
import { bestSkillBonus } from "../../engine/rules.js";

export default defineModule({
  id: "ypsilon14",
  title: "THE HAUNTING OF YPSILON 14",
  subtitle: "MOTHERSHIP · SCI-FI HORROR RPG · ONE SHOT",
  byline: "Module by D. G. Chapman, Tuesday Knight Games. Rules: Mothership 0e.",
  length: "One shot",
  ...
  hooks: {
    examineGoo(api) {
      const pc = api.pc();
      const bonus = bestSkillBonus(pc, ["Xenobiology", "Genetics", "Biology"]);
      const r = check(Math.min(99, pc.stats.intellect + bonus));
      api.say(r.success ? "rollgood" : "rollbad",
        `XENOBIOLOGY · Intellect${bonus ? ` +${bonus}` : ""} rolled ${pad(r.value)}`);
    ...
    giovanniEncounter(api) {
      ...
      api.startCombat("giovanni", false);
    },
