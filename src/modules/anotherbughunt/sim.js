...
/* Scenario one, once hour five has been and gone. The LZ is open
   ground and stays walkable; everything inside the building does
   not. */
const GRETA = ["airlock", "commissary", "pantry", "freezer", "habitat",
  "armory", "command", "medbay", "garage", "apc", "ducts", "theatre"];
...
export const simHooks = {
  ...
  onTick(api, { mins }) {
    ...
    /* The storm runs from the moment the crew reach Heron and it
       never stops. It deliberately does NOT run during scenario
       one — every threshold below names a part of Heron Station,
       and Greta Base is a long evening on its own. See `beginStorm`
       and the note in index.js `onStart`.

       Accumulate rather than assuming a fixed step — the director
       skips fifteen minutes at a time and a fixed step would
       silently drop beats. */
    const acc = (f.storm_acc || 0) + (f.storm_started ? mins : 0);
    const hoursNow = f.storm_started ? Math.floor(acc / HOUR) : 0;
    const hoursWere = Math.floor((f.storm_acc || 0) / HOUR);
    if (f.storm_started) api.flag({ storm_acc: acc });
  ...
  onEnterRoom(api, { room, first }) {
    ...
    /* THE STORM STARTS HERE, NOT AT THE LZ. */
    if (room === "hangar" && !f.storm_started) simHooks.beginStorm(api);

    /* GRETA BASE GOES UNDER TOO.

       Hour five of the timeline says, in as many words, that Greta
       Base is flooding and whatever is still in it stays in it. It
       raised `flooded_gretabase` and nothing anywhere read the flag,
       so all thirteen rooms of scenario one stayed dry and walkable
       for the rest of the campaign — including the freezer, which
       holds the acid, the foil cap and the twenty-five litres of
       chemotherapy the whole cytotoxin thread depends on.

       A crew who left those behind and want them at hour six should
       have to swim for them. */
    if (f.flooded_gretabase && GRETA.includes(room)) {
      api.say("horror", "Greta Base is not flooding any more. It has flooded. The corridor you are standing in is chest-deep and moving, and the doors are holding water on the far side.");
      api.effects([{ time: 20 }, {
        save: "body", why: "the swim through Greta Base",
        onFail: [{ damage: "1d10", why: "you were under too long" }, { stress: 2, why: "you were under too long" }],
      }]);
      api.say("warden", "▌ Anything they left in this building costs a swim to go back for. Say what floats past.");
    }
  ...
  /* The countdown the players watch. Raised on arrival at the
     hangar rather than at `onStart`, so the ten hours are Heron's
     ten hours and scenario one is not spending them. */
  beginStorm(api) {
    if ((api.world().flags || {}).storm_started) return;
    api.flag({ storm_started: true, storm_acc: 0 });
    api.effects([{
      countdown: {
        id: "storm", minutes: 600, full: 600,
        label: "THE STORM",
        tick: "THE STORM · {left} minutes until it breaks.",
        onZero: [{ run: "stormBreaks" }],
      },
    }]);
    api.say("alarm", "THE STORM · ten hours. Somebody in the hangar has the modelling up on a slate and has been watching it since this morning.");
    api.say("warden", "▌ The clock starts now. It takes the map away an hour at a time and it does not care what anybody is in the middle of.");
  },

  /* ACID IS NOT INFINITE.

     The coated-rounds counter spends its magazine on both branches
     — `{ run: "spendCoated" }` on onBreak and onHold. The acid
     counter did not spend anything at all, so one vial of
     hydrofluoric acid was an unlimited weapon in the only place the
     module ever tests the scarcity it is built on. Every acid item
     declares `uses`, and the engine honours `uses` on the itemUse
     path (`throwAcid`) but not on the combat-counter path, so the
     shortfall was invisible outside a fight and total inside one.

     There are eight throws on this planet. That number is the
     module. */
  spendAcid(api) {
    const pc = api.pc();
    if (!pc) return;
    const held = pc.items || [];
    const id = ["tumbler", "acidvial", "acidcan"].find((i) => held.includes(i));
    if (!id) return;

    if (id === "acidcan") {
      const f = api.world().flags || {};
      const left = Math.max(0, (f.acidcan_left ?? 4) - 1);
      api.flag({ acidcan_left: left });
      if (left <= 0) {
        api.take(["acidcan"]);
        api.say("warden", "▌ The lead container is empty. That was the last of the four.");
      } else {
        api.say("system", `${left} throw${left === 1 ? "" : "s"} left in the container.`);
      }
      return;
    }

    api.take([id]);
    api.say("system", id === "tumbler"
      ? "The tumbler is empty. That was Resnick's whole message, and it has been spent."
      : "The vial is empty. Glass, and nothing in it.");
  },
