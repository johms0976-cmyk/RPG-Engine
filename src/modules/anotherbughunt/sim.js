...
const TIMELINE = [
  ...
  { hour: 10, floods: null, waves: 0.50, evac: "landed", note: "The storm breaks. The sky goes quiet. And every carcinid on this continent is now above ground." },
];
...
export const simHooks = {
  ...
  onTick(api, { mins }) {
    ...
    for (let h = hoursWere + 1; h <= hoursNow && h <= 10; h++) {
      ...
      api.say("alarm", `HOUR ${h} · ${beat.note}`);
  ...
  onEnterRoom(api, { room, first }) {
    ...
    const DROWNED = {
      reactor: "flooded_reactor", spillways: "flooded_spillways",
      tunnels: "flooded_tunnels", stairs: "flooded_spillways",
    };
  ...
  /* ---------------- route A puzzles ---------------- */
  orrery(api) {
    const crew = api.crew().filter((c) => c.alive !== false);
    if (crew.length < 4) {
  ...
  a1Hatch(api) {
    const pc = api.pc();
    const carrying = (pc.items || []).some((i) => api.items[i] && api.items[i].carc);
    if (carrying) {
