/* ============================================================
   ...
   FOUR LADDERS, because four things are tightening at once and
   collapsing them into one list would mean the storm waits
   behind the carcs until somebody happens to trigger a beat.

     weather   the storm, which is on a timer and indifferent
     siege     the carcs outside, while the Signal is up
     relief    the one beat that fires when the Signal goes down
     people    the colonists, who are watching and choosing

   WHY `relief` IS ITS OWN LADDER. An escalation track is
   strictly sequential — stage two cannot fire until stage one
   has. "The siege lifts" used to sit third on the `siege`
   ladder behind two beats gated on `!flag:signal_down`, which
   meant that a crew who killed the Signal early locked those
   two beats out forever and the payoff behind them could never
   arrive. The single most consequential thing the players can
   do had no acknowledgement at all. It now has its own track,
...
export const escalate = [
  ...
  /* ---------------- the relief ---------------- */
  {
    track: "relief", when: "flag:signal_down", label: "the siege lifts",
    effects: [{
...
/* Rolls the director may call. Every one carries a `reason`,
   ...
   THE SCHEMA, WHICH THESE THREE HAD WRONG. `rungCallRoll` reads
   `stat` for which one and `save` as a boolean for whether it is
   a Save rather than a Check — `stat: r.stat || "sanity"`. Two of
   these entries wrote the stat into `save`, so `stat` fell
   through to its default and the module's Body and Fear saves
   were both being rolled as Sanity. Ypsilon 14 has it right and
   is the reference. */
export const rolls = [
  {
    id: "footing", when: "room:dam", stat: "body", save: true, once: true,
    reason: "The wall is ten metres wide, the rain is going sideways, and there is sixty metres of nothing on your left.",
  },
  {
    id: "cold", when: "flag:power_out", stat: "body", save: true, once: true,
    reason: "The heating went with the lights and you have been wet since you landed.",
  },
  {
    id: "nerve", when: "flag:endgame", stat: "fear", save: true, once: true,
...
/* Moments a threat may legitimately come through the door.
   ...
   `wave_pending` is now cleared at the top of every storm hour
   in sim.js, so `hangar_wire` is armed for the hour a wave
   actually landed and not for the rest of the session. */
export const attacks = [
  ...
  /* WHAT CAME BACK. The faction that went out because the crew went
     somewhere else does not simply stop existing — two of them walk
     back to the wire several hours later, in uniform, at Stage 4.
     The `grunt` threat was fully statted and placed nowhere. */
  {
    id: "returned", threatId: "grunt", when: "flag:grunts_loose and flag:reached_heron",
    room: "hangar", count: 2, distance: 20, once: true,
    reason: "Two people in colony fatigues have come up to the wire from the direction the other team went, and they are not answering, and one of them is carrying a rifle by the barrel.",
  },
  {
...
export const listeners = [
  ...
  {
    id: "foil", phrases: ["tin foil", "tinfoil", "foil hat", "foil cap", "the hat"],
    when: "flag:knows_airborne and !flag:foil_proven",
    effects: [{ say: "Two men on this planet folded one and both of them are dead, which is either the whole argument against it or no argument at all. Nobody has actually tested it.", tone: "warden" }],
  },
  {
  ...
  /* The other half of that one. A crew who have retaken the tower
     and are still being told the radio is dead is a crew being
     lied to by the software. */
  {
    id: "leave_open", phrases: ["let's leave", "get off this rock", "just go", "call the dropship"],
    when: "flag:comms_up and !flag:evac_called",
    effects: [{ say: "The channel is clean and the dropship is listening. Somebody has to actually make the call.", tone: "good" }],
  },
];
