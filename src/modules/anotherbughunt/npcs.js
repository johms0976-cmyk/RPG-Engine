...
const ACID_COUNTER = {
  ...
  /* Both branches spend a charge, the way COATED_COUNTER does. A
     throw that missed is still a throw that left the container. */
  onBreak: [
  ...
  onHold: [
    { say: "It gets a claw up and takes the splash on the shell of it. The shell smokes. It does not stop coming.", tone: "warden" },
    { run: "spendAcid" },
  ],
};

/* THE OTHER ANSWER, AND WHY IT IS A COUNTER.

   Coated rounds carried a `grants` block that could never match a
   combat roll's tags (see items.js). Acid was already modelled the
   right way — a named button that appears in a fight only when
   somebody is carrying the answer — so coated ammunition is now the
   same shape, with a magazine that empties. It is the Study Group's
   payoff and it should look like one on the screen. */
const COATED_COUNTER = {
  id: "coated", label: "Put the coated rounds into it", when: "has:coatedammo",
  hint: "Edem's compound", whenText: "Ordinary ammunition, and you have seen what that does.",
  roll: "combat", endsCombat: false,
  onBreak: [
    { say: "The rounds go in and keep going, through the plate and through whatever the plate was protecting. It does not reallocate this time.", tone: "good" },
    { damage: "4d10", target: "threat" },
    { run: "spendCoated" },
  ],
  onHold: [
    { say: "The burst goes wide across the shell and takes a strip of it off. That is more than any round has managed all week, and it is not enough.", tone: "warden" },
    { run: "spendCoated" },
  ],
};
...
export const threats = {
  carc: {
    ...
    counters: [ACID_COUNTER, COATED_COUNTER],
    onSlain: [
  ...
  abara: {
    ...
    counters: [
      ...
      COATED_COUNTER,
      {
        id: "bandolier", label: "Shoot the grenades on its chest",
        /* NOT `tag:WPN`. The `tag:` predicate reads items[id][arg] —
           it is for boolean properties like `cuts` and `acid`. A
           weapon's tag is the *string* "WPN", so `tag:WPN` asked for
           items[id].WPN and was false for every weapon in the game,
           which quietly deleted the best option in scenario one. */
        when: (ctx) => !!ctx.pc && ctx.pc.items.some((i) => ctx.items[i] && ctx.items[i].tag === "WPN"),
  ...
  hinton: {
    ...
    tactics: "loudest", morale: 0.2, start: "court", ambushes: false,
    note:
  ...
  retinue: {
    ...
    tactics: "nearest", count: 2, start: "court", ambushes: false,
  ...
  noble: {
    ...
    tactics: "random", ambushes: false,
    /* PLACED, AT LAST. The noble was fully statted — including the
       Assimilation attack that is the only caller of the
       `assimilate` hook — and no room anywhere put it in the world,
       so the stat block and the hook were both dead code. It is now
       in the Court with `ambushes: false`: it is present, it does
       not start anything, and the crew decide. */
    note:
