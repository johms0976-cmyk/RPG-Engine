...
export function register(line) {
  ...
  /* A character saying something out loud at the table. It is not
     the module narrating and it is not the Warden speaking, and it
     must not read as either — the whole weight of a share is that a
     person chose to say it and might be lying. */
  if (line.kind === "share") return "spoken";
...
function audienceBadge(line, crew, myPcId) {
  /* A share is public by construction. Badging it would be worse
     than useless: it is the one line on the screen everybody is
     meant to have. */
  if (line.kind === "share") return null;
          ...
          return (
            <React.Fragment key={line.id}>
              ...
              <p className={`feedlog-line k-${line.kind} is-${reg}${line.phantom ? " is-phantom" : ""}${badge ? " is-private" : ""}`}>
                ...
                {/* WHICH RUNG SAID THIS.

                    Carried on `extra.director` by useDirector and shown
                    only when the reader has asked for stamps, because it
                    is a debugging affordance rather than fiction.

                    2.7.0 put the rung in the assisted strip, which shows
                    what the director is *about* to do. This is the other
                    half and the more useful one: after a session that
                    felt wrong, the feed is the only record of what
                    actually ran, and "the room went quiet for twenty
                    minutes" is unanswerable without it. */}
                {showStamps && line.extra && line.extra.director && (
                  <span className="feedlog-rung"
                    title={`The director's ${line.extra.director} rung wrote this line.`}>
                    {line.extra.director}
                  </span>
                )}
                {badge && (
