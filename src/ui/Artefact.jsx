...
export function Artefact({ handout, id, onOpen, flat = false }) {
  ...
  /* The rack is thumbnails inside a <button>, so a transport cannot
     live there: nesting controls in a button is invalid, and every
     tap on the reels would open the artefact instead of moving the
     tape. It belongs to the held-up view, which is also the right
     place for it — you pick the tape up, and then you play it.

     Listening comes above the transcript because listening is the
     act and reading is the fallback: for a player who cannot hear
     it, a table too loud to use it, or the second time somebody
     wants the exact wording without sitting through three minutes. */
  if (flat) {
    if (!handout.audio) return body;
    return (
      <>
        <div className={`artefact is-${style} is-deck`} data-handout={id}>
          <div className="artefact-label">{handout.label}</div>
          <TapeDeck
            src={handout.audio}
            secs={handout.audioSecs || 0}
            label={handout.label}
            note={handout.audioNote}
          />
        </div>
        {body}
      </>
  ...
  return (
    <button type="button" className="artefact-shell" onClick={onOpen}
      ...
      {handout.audio && <span className="artefact-more">▶ Playable</span>}
    </button>
  );
}
