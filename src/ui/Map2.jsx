...
export function MapV2({ mod, w, crew = [], activeId, onGo, wardenView = false, objectives = [] }) {
  ...
  const cameFrom = useRef(w.room);
  useEffect(() => {
    const from = cameFrom.current;
    cameFrom.current = w.room;
    if (from === w.room || !from) return undefined;
    if (!floor.pos[from] || !floor.pos[w.room]) return undefined;
    setTrail({ from, to: w.room, at: Date.now() });
