...
export function TitleSequence({ mod, onBegin, onGather, onGatherAlone, onRemote, onQuick, onBack }) {
  const lines = useRef([
    { t: "PWR ....... NOMINAL", k: "ok" },
      ...
      {done && (
        <>
          ...
          <div className="title-actions">
            ...
            {/* THE EMPTY CHAIR, offered as a different kind of evening
                rather than as a setting. Choosing it is permanent for
                the session: this device becomes the shared screen and
                there is no Warden deck to tab back to. */}
            {onGatherAlone && (
              <Btn
                /* Without a socket there is no GATHER above it, so the
                   empty chair is the leading choice rather than the
                   second one. It is the reason most people opening
                   this page with no server are here. */
                kind={onGather ? "default" : "primary"}
                onClick={onGatherAlone}
                hint={onGather
                  ? "nobody runs it — the module does"
                  : "nobody runs it — one screen, pass it round"}>
                NOBODY IS THE WARDEN
              </Btn>
            )}
            {/* THE THIRD DOOR. Both buttons above assume everybody is in
                the room. Most tables are not, and this engine can run a
                table that isn't with nothing running anywhere — which
                was true for three releases while being reachable only
                by hand-editing the URL. A configuration nobody can find
                is a configuration nobody has. */}
            {onRemote && (
              <Btn onClick={onRemote} hint="direct connections — nothing to install, no server">
                SOMEBODY IS NOT IN THE BUILDING
              </Btn>
            )}
            <Btn kind={onGather || onGatherAlone ? "default" : "primary"} onClick={onBegin}>
