  ...
  React.useEffect(() => {
    ...
    const c = recordSession(campaignId, {
      ...
      /* THE FACTS THIS TABLE MADE TRUE. `recordSession` harvests
         ...
         is unaffected. */
      world: w,
      /* THE MODULE, so `recordSession` can reduce this evening to a
         digest. Analytics spanning several sessions cannot be
         computed from six stored worlds — nobody keeps those — so
         the counts are taken once, here, while the world still
         exists. See engine/analytics.js. */
      mod,
      feed,
    });
