...
export default function Library({ modules, broken = [], onPick, onResume, onWardenTools, onWrite, onPaper, onShelfChange }) {
    ...
    <div className="center-screen" style={{ alignItems: "flex-start", padding: "28px 16px" }}>
      <div style={{ width: "100%", maxWidth: 880 }} className="stack">
        ...
        <div className="btn-row">
          ...
          {/* Beside loading one rather than behind Warden tools. Writing
              a module and loading somebody else's are the same door in
              two directions, and an author who has just been told they
              can load a `.mship` file should be able to see, in the same
              row, that they can also write one. */}
          {onWrite && (
            <Btn kind="ghost" className="inline" onClick={() => onWrite(null)}>
              Write a module
            </Btn>
          )}
        </div>
            ...
            <Panel key={m.id} title={card.title} icons={m.portable ? `${card.length} · LOADED` : card.length}>
              <div className="stack">
                ...
                <div className="btn-row">
                  ...
                  {/* Offered on every module, bundled or loaded, and
                      without the `bad` guard the New game button carries.
                      A module that will not load is exactly the one an
                      author wants to read on paper. */}
                  {onPaper && (
                    <Btn kind="ghost" className="inline small" onClick={() => onPaper(m)}>
                      Print it
                    </Btn>
                  )}
                  {/* Loaded modules only, and that is not a policy — a
                      bundled module carries `hooks`, the portable format
                      cannot, and the editor would refuse it at the door.
                      Offering a button that always fails is worse than
                      not offering one. */}
                  {m.portable && onWrite && (
                    <Btn kind="ghost" className="inline small"
                      onClick={() => onWrite(exportInstalled(m.id) || m.id)}>
                      Edit
                    </Btn>
                  )}
                  {m.portable && (
