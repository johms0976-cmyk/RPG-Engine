...
const TIERS = Object.keys(SKILL_TREE);

/* Every step this screen knows how to draw, filtered to the ones the
   loaded ruleset says it has. The wizard was already a list of steps,
   which made this the one creation surface that did not need
   restructuring — only a filter. Review is not a ruleset step: it is
   this screen's own last page and is always there.

   `flavour` has no page here. On the phone the trinket and patch are
   rolled and shown on Review rather than being a step of their own,
   which is why it is absent from this list and present in the desk
   creator's. */
const ALL_STEPS = [
  { id: "name", label: "Name" },
...
const STEPS = [
  ...ALL_STEPS.filter((s) => RULESET.create.steps.includes(s.id)),
  { id: "review", label: "Review" },
];

const has = (id) => STEPS.some((s) => s.id === id);

/* What a class is when a system has none. Same shape, no bonuses, so
   every read below stays identical in both cases. */
const EMPTY_CLASS = {
...
export default function CreatorPhone({ mod, onOffer, onBack, playerName }) {
  ...
  const [draft, setDraft] = useState(() => ({
    ...
    cls: has("class") ? (Object.keys(CLASSES)[0] || null) : null,
    stats: rollStats(),
    picks: [],
    spent: [],
    loadout: null,
  ...
  /* How far the wizard will let somebody jump. Computed from the
     step LIST rather than from fixed indices — the old version said
     "3" and "4", which are the skills and kit pages only while every
     ruleset has all five. */
  const stepAt = (id) => STEPS.findIndex((x) => x.id === id);
  const furthest = (() => {
    if (has("name") && !draft.name.trim()) return Math.max(0, stepAt("name"));
    if (has("skills") && (needPick > 0 || left > 0)) return stepAt("skills");
    if (has("loadout") && !draft.loadout) return stepAt("loadout");
