 ...
 */
export default function Play({ g, core, onQuit, net, onWhisper, tableHandout }) {
  const {
  ...
  const rightCol = (
    <>
      <Panel title="Location" icons={fmtClock(w.clock)}>
        <MapV2 mod={mod} w={w} crew={crew} activeId={pc.id} wardenView={isWarden}
          onGo={(id) => {
            ...
            <div className="btn-grid">
              {crew.filter((c) => c.alive !== false && c.id !== pc.id).map((c) => (
                <Btn key={c.id} kind="solid" className="small"
                  onClick={() => { g.giveItem(giving, c.id); setGiving(null); }}>
                  {c.name}
                </Btn>
              ))}
            </div>
      ...
      {tableHandout && mod.handouts[tableHandout] && propDown !== tableHandout && (
        <FullScreen title="On the table" tone="artefact"
          ...
          <p className="clue-meta" style={{ textAlign: "center", marginTop: 12 }}>
            The Warden is holding this up. Putting it down only puts it down for
            you — it stays in your Evidence, and on the table screen.
          </p>
        </FullScreen>
      )}
