...
import { canFire, shotsForAttack, isTrainedShooter, reloadCost, liveEnemies, currentTurn } from "../engine/combat.js";
  ...
  const rightCol = (
    <>
      <Panel title="Location" icons={fmtClock(w.clock)}>
        <BaseMap mod={mod} w={w} onGo={(id) => {
          const ex = exits.find((e) => e.to === id);
          if (ex) doMove(ex);
        }} threatRooms={Object.entries(w.threats).filter(([, t]) => !t.dead && t.loc).map(([, t]) => t.loc)} />
      </Panel>
        ...
        <div className="stack">
          {!combat && (
            <>
              ...
              {(room.devices || []).map((id) => mod.devices[id] && (
                <Btn key={id} kind="solid" onClick={() => setDevice(id)}>{mod.devices[id].label || mod.devices[id].title}</Btn>
              ))}

              {(mod.actions || []).concat(room.actions || []).filter((a) => !a.when || g.api.ctx() && true).map((a) => (
          ...
          {combat && (
            <>
              ...
              <ActionGroup label="Manoeuvre">
                ...
                {target && (mod.threats[target.threatId].counters || []).map((k) => (
                  <Btn key={k.id} kind="solid" disabled={!myTurn || actor.actions <= 0}
                    onClick={() => useCounter(k.id)}>{k.label}</Btn>
                ))}
                <Btn kind="danger" disabled={!myTurn} onClick={fleeCombat}>Run</Btn>
