    ...
    <>
      {open && (
        <div className="wdeck-drawer" role="region" aria-label="Warden controls">
          ...
          <div className="wdeck-body">
            ...
            {open === "world" && <WorldTab g={g} />}
            {open === "sound" && <SoundTab g={g} net={net} />}
            {open === "whispers" && <WhisperTab net={net} crew={crew} />}
      ...
      <div className="wdeck-bar">
        ...
        <Btn kind={open ? "primary" : "solid"} className="inline"
          onClick={() => setOpen(open ? null : "crew")}
          aria-expanded={!!open}>
...
/* ============================================================
   PROPS — handouts as objects rather than scrollback.
   ============================================================ */
function PropsTab({ g }) {
  const { mod, w, crew, warden } = g;
  ...
  if (!ids.length) return <p className="clue-meta">This module has no handouts.</p>;

  return (
    <div className="stack">
      <p className="clue-meta" style={{ margin: 0 }}>
        Who has read what, and what is currently in the middle of the table.
      </p>
      <div className="clues">
        {ids.map((id) => {
          const h = mod.handouts[id];
          const s = seen[id];
          const readers = s ? s.by.map((pid) => (crew.find((c) => c.id === pid) || {}).name).filter(Boolean) : [];
          ...
          return (
            <div key={id} className={`clue${s ? "" : " is-resolved"}`} style={{ display: "block" }}>
              ...
              <div className="btn-row" style={{ marginTop: 6 }}>
                <Btn kind={onTable ? "accent" : "ghost"} className="inline small"
