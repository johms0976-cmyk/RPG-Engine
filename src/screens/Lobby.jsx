...
import CarryForward from "./CarryForward.jsx";
import CampaignPanel from "./CampaignPanel.jsx";
import { getCampaign } from "../engine/campaign.js";
...
/* ============================================================
   HOW LONG THIS EVENING IS

   Not a countdown and not a limit. It is the table telling the
   director what time people have to leave, which is the one fact a
   person behind a screen has always had and the empty chair never
   did — see `rungLastCall`. At the declared time it says so once,
   out loud, and then stops opening new threads: no new
   conversations, no revisited clues, no fresh pressure. The
   module's own escalations and its own endings keep running.

   Absent by default, and that is the point rather than an
   oversight. A table that has not asked to be steered is never
   steered, so the "no limit" option is not a null state to be
   tidied away later — it is the honest answer for most evenings.
   ============================================================ */
const LENGTHS = [
...
function SessionLength({ mins = 0, onMins }) {
  return (
    <Panel title="How long have you got">
      <div className="btn-row" style={{ flexWrap: "wrap" }}>
        {LENGTHS.map((l) => (
          <Btn
            key={l.mins}
            kind={mins === l.mins ? "accent" : "ghost"}
            className="inline small"
            onClick={() => onMins(l.mins)}
          >
            {l.label}
          </Btn>
        ))}
      ...
      <p className="clue-meta" style={{ margin: "8px 0 0" }}>
        {mins > 0
          ? "At that point the table is told the time, once, and nothing new gets started — "
            + "the situation still finishes, and it can still finish early."
          : "Nobody gets steered toward an ending. Pick a length if people have to be somewhere."}
      </p>
    </Panel>
  );
}
...
export default function Lobby({
  ...
  wardenless = false, ready = {},
  remote, onRemote, rtc,
  safety, onSafety,
  sessionMins = 0, onSessionMins,
  campaignId = null, onCampaign, onCarry,
}) {
  ...
  return (
    <div className="lobby">
        ...
        <div className="stack">
          ...
          {onRemote && (
            <label style={{ display: "flex", gap: 10, alignItems: "flex-start", cursor: "pointer" }}>
              ...
              />
              <span>
                <strong style={{ fontFamily: "var(--display)", letterSpacing: "0.08em" }}>
                  SOMEONE IS NOT IN THE BUILDING
                </strong>
                <br />
                <span style={{ fontSize: 12, color: "var(--bone2)" }}>
                  The table runs over direct connections instead of the wifi
                  relay: everyone joins by swapped code, wherever they are.
                  Phones on the wifi can join this way too — the two kinds of
                  table just cannot run at once.
                </span>
              </span>
            </label>
          )}
      ...
      {remote && rtc && <RemotePanel rtc={rtc} />}
      ...
      <div className="lobby-grid">
        ...
        <div className="stack">
          ...
          {onSessionMins && <SessionLength mins={sessionMins} onMins={onSessionMins} />}
          {/* Beneath the length, because it is the less urgent of
              the two questions: how long tonight runs affects
              tonight, and whether tonight is remembered does not. */}
          {onCampaign && <CampaignPanel campaignId={campaignId} onCampaign={onCampaign} />}
          {/* THINGS THIS TABLE MADE TRUE LAST TIME.

              Only appears when a campaign is selected AND it has
              facts in it, so a one-off session never sees it and a
              first session in a new campaign never sees it either.
              Renders null on its own when there is nothing to ask
              — see screens/CarryForward.jsx. */}
          {onCarry && campaignId && (
            <CarryForward
      ...
      <div className="lobby-foot">
        {wardenless && (
          <div className="note-box" style={{ marginBottom: 10 }}>
            <strong>NOBODY IS THE WARDEN.</strong>
            <div style={{ margin: "6px 0 0", fontSize: 12 }}>
              Characters are let in as they arrive — there is no queue,
              because there is nobody to look at one. This screen becomes
              the table&apos;s screen when it starts and stays that way:
              there is no Warden deck to come back to. Any phone can start
              it, and so can this.
              {Object.values(ready).filter(Boolean).length > 0 && (
                <> · <strong>{Object.values(ready).filter(Boolean).length} ready</strong></>
              )}
            </div>
          </div>
        )}
        ...
        <div className="btn-grid">
          <Btn kind="accent" disabled={short > 0} onClick={onBegin}>
            {short > 0
              ...
              : (wardenless
                ? `Start with ${roster.length} character${roster.length === 1 ? "" : "s"}`
