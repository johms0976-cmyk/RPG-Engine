...
export default function TableFar({ g, peers, spotlight, safetyCall, vote, reactions = [], assistOffers = [] }) {
  ...
  const t = tempoOf(w);
  /* THE SAME TWO REDACTIONS TABLEVIEW MAKES, and for the same
     reason: this is the identical shared screen at couch distance,
     not a different kind of surface. Anything true of one is true
     of the other, and it would be a poor joke if the fix for a
     leak on a desk left it in place on a television. See the
     header of TableView.jsx for the argument. */
  const shown = combat ? redactCombat(combat, mod, crew) : null;
  ...
  if (t.held) {
    ...
    /* `order` entries are {side, id} and carry no name, so the
       enemy branch used to fall through to `turn.id` — a raw uid
       like "unseen#0", which names the threat. Resolved against
       the redacted enemy list, an unseen thing reports the label
       its module wrote for it. */
    turnWho = turn.side === "pc"
      ? (nameOf(turn.id) || turn.id)
      : ((enemies.find((e) => e.uid === turn.id) || {}).name || "SOMETHING");
  ...
  /* `visibleFeed` with a null viewer drops wardenOnly AND every
     addressed line, which is the set the room is allowed to hear.
     The old `!l.wardenOnly` caught the first and missed the
     second, so a description written for two people in another
     compartment could surface as one of the three lines on the
     television. */
  const lines = visibleFeed(feed, VIEW.PLAYER, null)
    .filter((l) => SPOKEN.includes(l.kind))
    .slice(-3);
      ...
      <Takeover label={`Combat · round ${combat.round}`} title={turnWho}>
        <ul className="tv-enemies">
          {enemies.map((e) => (
            <li key={e.uid}>
              <span>{e.name}</span>
              <span className="tv-wounds">{e.hidden ? "—" : `${e.wounds}/${e.maxWounds}`}</span>
            </li>
          ))}
        </ul>
      </Takeover>
