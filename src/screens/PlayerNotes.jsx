...
import { PANIC_TABLE } from "../engine/rules.js";
import { Disclosure } from "../ui/Hint.jsx";
...
export default function PlayerNotes({ mod, w, crew, pc, safety }) {
  ...
  return (
    <div className="stack" style={{ paddingBottom: 24 }}>
      ...
      {/* ---- THE TABLE THE GAME IS ACTUALLY ABOUT ----

          Mothership's Panic Effect table is the mechanism that
          gives Stress its meaning, and until now the only way a
          player could see any of it was to land on a row. That is
          a strange thing to hide: it is printed in the players'
          book, every experienced player at the table already knows
          the shape of it, and the dread comes from knowing that
          Heart Attack is on there — not from being surprised by it.

          Withheld information that only the new players lack is
          not tension. It is an unequal table.

          Read straight off PANIC_TABLE so it cannot drift from
          what the engine rolls on. Folded shut, because a wall of
          nineteen rows is not something anybody wants open. */}
      <Panel title="The Panic Effect table">
        <p className="clue-meta" style={{ margin: "0 0 8px" }}>
          2d10 plus your Stress, minus your Resolve. High is bad. This is the
          one roll in the game where you want the total low.
        </p>
        <div className="stack">
          {PANIC_TABLE.map((row) => (
            <Disclosure key={row.name} summary={row.name} meta={`up to ${row.max}`}>
              <p style={{ margin: 0 }}>{row.t}</p>
            </Disclosure>
          ))}
        </div>
      </Panel>
