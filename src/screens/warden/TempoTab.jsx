...
import { tempoOf, sceneHolder, scenePosition, sceneCost, sceneSpent, sceneHolders, laneSplit } from "../../engine/tempo.js";
/* Deliberately only the switch's own state. The scoring stays out of
   every screen in the app — see rule 2 in engine/floor.js, which
   tests/floor.test.js enforces by reading these imports. */
import { floorOf } from "../../engine/floor.js";
import { pacingOf, pacingNote, soonestClock } from "../../engine/pacing.js";
...
export default function TempoTab({ g }) {
  ...
  /* Lanes: with the party split, several people hold their own room
     at once, and a panel that names one of them is lying about the
     other. See LANES in engine/tempo.js. */
  const holders = sceneHolders(t, crew, w);
  const split = laneSplit(t, crew, w);

  /* The real clock against the fiction's. Almost always silent —
     see engine/pacing.js for why that restraint is the feature. */
  const soon = soonestClock(w);
  /* THE RATE HAS TO COME FROM THE MODULE.

     `pacingOf` takes a `rate` and defaults to 1.3 fiction-minutes
     per real minute when it is not given one. This call did not
     give it one, while engine/director.js reads
     `mod.director.rate` correctly for the same computation — so
     the two halves of the pacing system disagreed about every
     module that set a rate.

     ANOTHER BUG HUNT declares 6. Measured against 1.3, a table
     running it exactly on the module's own intended pace scores a
     ratio of about 460%, which is well past RUSH_OVER, so from
     minute twenty-five onwards the Warden's deck told them
     the window was closing before the table got there — every
     session, permanently, on a module that was doing fine. */
  const rate = mod?.director?.rate;
  const pacing = pacingOf({ startedAt: w.startedAt, clock: w.clock, ...(rate ? { rate } : {}) });
  const note = pacingNote(pacing, soon ? { soonest: soon.left, label: soon.label } : {});

  return (
    <div className="stack">
      {note && (
        <div className={`pacing-note is-${pacing.state}`} role="status">
          <span className="pacing-note-k">PACING</span>
          <span>{note}</span>
        </div>
      )}

      {split && (
        <div className="pacing-note is-lanes">
          <span className="pacing-note-k">SPLIT</span>
          <span>
            The crew is in {new Set(crew.filter((c) => c.alive !== false).map((c) => c.room || w.room)).size} places,
            so the round is running one spotlight per room — {holders.map((id) => {
              const pc = crew.find((c) => c.id === id);
              return pc ? pc.name : "…";
            }).join(", ")} are all up at once. They still settle into one round when the ring wraps.
          </span>
        </div>
      )}
      {/* ---------------- the raised hand ---------------- */}
      ...
      {/* ---------------- the floor ----------------
          ...
          One switch, and no numbers. There is deliberately nothing
          here showing who has acted how often: the moment that is on
          a screen it becomes a thing to optimise, and the eager
          player is not being malicious — they are being enthusiastic.
          See engine/floor.js. */}
      <div>
        <Label>MIND WHO HAS NOT SPOKEN</Label>
        <div className="btn-row">
          <Btn kind={floor.on ? "accent" : "ghost"} className="inline small"
            onClick={() => warden.floor(true)}>On</Btn>
          <Btn kind={!floor.on ? "accent" : "ghost"} className="inline small"
            onClick={() => warden.floor(false)}>Off</Btn>
        </div>
        <p className="clue-meta" style={{ margin: "6px 0 0" }}>
          Rounds open with whoever has had least of the room, a phone that
          has gone a long time without a go gets a quiet nudge, and anyone
          running away with it waits a beat when somebody else is trying.
          Nobody is ever named and no counts are shown, here or anywhere.
          A player who hangs back twice stops being asked. Off by default.
        </p>
      </div>
    </div>
  );
