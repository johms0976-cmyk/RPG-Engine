...
function Cartridge({ mod, slotName, resume, onEject, onRestart }) {
  ...
  React.useEffect(() => {
    if (!mod.ship || core.state.ship || !crew.length) return;
    core.do.install(makeShip(mod.ship));
  }, [mod.ship, core.state.ship, crew.length, core]);
  ...
  const net = useHost({ g, mod, phase, enabled: HOSTING });

  // An approved character joins the crew here, on the host, with a fresh
  // id and its session bookkeeping cleared.
  const acceptSubmission = React.useCallback((entry) => {
    gRef.current.addCrewMember(adoptCharacter(entry.character, newPcId()));
    net.resolveSubmission(entry, true);
  }, [net]);
  ...
  if (HOSTING && view === "board") {
    ...
    screen = <TitleSequence mod={mod} onBegin={() => setPhase("create")} onQuick={quick} onBack={onEject} />;
