...
function Cartridge({ mod, slotName, resume, onEject, onRestart }) {
  ...
  /* ============================================================
     ...
     ============================================================ */
  const [tableMode, setTableMode] = useState("warden");
  const wardenless = HOSTING && tableMode === "wardenless";
  ...
  const directorOn = HOSTING && phase === "play" && (tableMode === "wardenless" || assist);
  ...
  /* ============================================================
     ...
     At four, airtime broadly self-corrects. At five and six it does
     not: the quiet two stop trying, read it as the app being broken,
     and put the phone down. So the default inverts at exactly the
     size where the argument for it inverts.
  ...
  if (wardenless && phase === "play" && crew.length) {
    screen = (
      <TableView
    ...
    screen = (
      <>
        ...
        <TitleSequence
          ...
          onGatherAlone={HOSTING ? () => { setTableMode("wardenless"); setPhase("lobby"); } : null}
          onQuick={quick}
          onBack={onEject}
        />
      </>
