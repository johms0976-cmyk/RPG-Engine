...
const RemoteJoin = lazy(() => import("./net/RemoteJoin.jsx"));
...
/* ============================================================
   Which of the three things is this tab?
     ...
     ?mode=join   a phone joining a REMOTE table by pasted code —
                  no table server exists, so probing would only
                  find silence and fall back to the wrong thing
...
function Boot() {
  const [role, setRole] = useState(
    mode === "join" ? "remote" : mode === "host" || mode === "solo" ? "solo" : null,
  );
  ...
  return (
    ...
    <Suspense fallback={null}>
      {role === "remote" ? <RemoteJoin /> : role === "client" ? <ClientShell /> : <App />}
    </Suspense>
  );
}
