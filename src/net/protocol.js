...
import { tempoVerdict, tempoOf, sceneHolder } from "../engine/tempo.js";
...
export function decideIntent({
  ...
  const brake = tempoVerdict({
    w: game.w, action: job.action, pcId: job.asPc, now, lastActed,
  });
...
export function waitingRoom({ game, claims, currentTurn, lastActed = {}, now = Date.now() }) {
  ...
  for (const pc of game.crew) {
    ...
    if (holder) {
      out[pc.id] = holder === pc.id
        ? { ...base, state: "acting" }
        : { ...base, state: "held", why: "scene" };
      continue;
    }
