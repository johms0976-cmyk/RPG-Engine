...
import { floorVerdict } from "../engine/floor.js";
...
/* "I tapped and nothing happened."
   ...
   The gate on the phone swallows a second tap while the first is
   still in the air (see useIntentGate). Locally that is right and
   the player is told so. But the *host* never heard about it, and
   a swallowed tap is the single unambiguous signal that somebody
   wanted the floor and was outrun for it — the only one of the
   three failure shapes in engine/floor.js that was invisible.

   It carries no action of its own and can move nothing. The relay
   attaches the character; the phone does not get to claim one. */
export const C_TAP = "tap";         // { action } -> host { clientId, asPc }
/* THE EMPTY CHAIR'S TWO MESSAGES.

   With a Warden present, "everyone is here" and "go" are one
   person's judgement and need no protocol. With the chair empty
   they have to come from the phones, because there is nobody else
   to ask — so a phone can say it is ready, and any phone can start
   a table that is ready.

   Deliberately *any* phone rather than a nominated one. Making the
   first player to join the starter reintroduces a host role
   through the back door and gets it wrong, because the person who
   joined first is not necessarily the person who knows everyone
   has arrived. The host re-checks the crew minimum, so the worst a
   premature tap can do is nothing. */
export const C_READY = "ready";     // { ready }
export const C_START = "start";     // {}
/* CLEARING THE CARD, FROM ANY PHONE.

   The card has always been anonymous going up. Coming down it was
   not: `clearSafety` was a button on the host device, which in
   wardenless mode is the screen in the middle of the table. Whoever
   reaches for it is visibly the person handling it, and if the card
   was about them they have to reach past it to say so.

   So every phone can clear it, and the host never says which one
   did. This carries no identity for the same reason C_SAFETY does
   not. */
export const C_CLEARSAFETY = "clearsafety"; // {}
/* THE TABLE DECIDING SOMETHING. See engine/vote.js — one primitive,
   five topics, and never the safety pause itself. */
export const C_VOTE = "vote";       // { choice }
export const C_OPENVOTE = "openvote"; // { topic }
/* "What do I see?" — the half of table talk that had no button.

   Players could act on a room, search it and interrogate the people
   standing in it, but could not ask the *situation* anything. With a
   Warden that question is answered forty times an evening. It
   returns nothing new: the host answers from the room the crew has
   already entered and the clues they have already pinned. */
export const C_LOOK = "look";       // { about }
/* "That is not what I meant."

   A director move addressed to one player can be waved off by that
   player, within a few seconds, and the ledger does not count it
   against them. With a person you simply say so; without one there
   has to be a button, or the system is something imposed rather
   than something offered. */
export const C_DISPUTE = "dispute"; // { moveId }
/* "Not that" — the same correction, for the lines nobody was
   addressed by.

   C_DISPUTE only ever worked on a Move carrying a `pcId`, which
   excludes `describe`, `atmosphere`, `npcSay`, `clock` and
   `callback` — the majority of what a wardenless table hears. This
   attributes to the LAST MOVE instead of to a player, and takes a
   second voice to carry, because a line said to the room can only
   be judged by the room. See engine/objection.js for the
   threshold and the reasoning behind it.

   Carries no identity onward for the same reason C_SAFETY does
   not. The relay stamps `asPc` so the host can tell two people
   apart from one person tapping twice; the host publishes a count
   and never a name. */
export const C_NOTTHAT = "notthat"; // {}

/* ============================================================
   THE TABLE MAKES A FACT TRUE — C_PROPOSE / C_SECOND

   The counterpart to C_NOTTHAT, and deliberately the same shape.
   C_NOTTHAT is the table saying "not that" about a line the
   director said; these are the table saying "and also this"
   about a fact the module never wrote.

   Both take a second voice and both take it for the same reason
   (INV-10): a thing addressed to the room can only be judged by
   the room. A ruling is addressed to the room by construction —
   it changes what the room says to every phone — so one player
   cannot carry it alone.

   `C_PROPOSE` carries the player's own typed sentence and nothing
   else that could become content. The host does not rewrite it,
   the relay does not read it, and `engine/tableRuling.js` passes
   it to `makeRuling` verbatim. INV-1 and INV-6 are untouched:
   the only thing this pair decides is WHEN a human's sentence
   becomes true, never what it says.

   Unlike C_NOTTHAT these DO carry identity onward to the host,
   and that is not an oversight. The whole mechanism is that the
   proposer cannot second themselves, which requires the host to
   tell two people apart. What the host publishes is still a
   count and never a name — see engine/tableRuling.js. */
export const C_PROPOSE = "propose"; // { text, scope, room, subject }
export const C_SECOND = "second";   // {}
...
/**
 * The peer mode this table may actually have, given who is running it.
 *
 * `dark` promises that player-to-player whispers pass through nobody.
 * With a Warden that is true enough to say out loud: the router is the
 * Warden's own browser, and the Warden is already the person the table
 * has handed the whole fiction to.
 *
 * With the chair empty it is not true and cannot be made true. The
 * host is a device belonging to somebody sitting at this table who is
 * also playing, chosen because they happened to open the tab — and
 * the table may not even have noticed whose it is. Attaching the word
 * "dark" to that arrangement is a promise the software cannot keep.
 *
 * So it downgrades, and it is *reported* rather than silently
 * softened, exactly as `requestPeerMode` already does. `seen` is
 * honest in both modes: the Warden's slot is empty, so nothing is
 * displayed, but nothing was promised either.
 */
export function allowedPeerMode(mode, tableMode) {
  const m = PEER_MODES.includes(mode) ? mode : "seen";
  if (tableMode === "wardenless" && m === "dark") return "seen";
  return m;
}
...
/** How a table is being run. Chosen once, at the title, and fixed
    for the life of the session — see WARDENLESS below.

      warden      a person has the deck. Everything as it always was.
      wardenless  nobody does. The host device shows the table screen
                  and the director runs the module.

    It is on the snapshot so a phone can say something truer than
    "waiting for the Warden" when there is not going to be one. */
export const TABLE_MODES = ["warden", "wardenless"];
...
export const PLAYER_ACTIONS = new Set([
  ...
  "jumpIn",
  /* Speaking, at the table, about a thing only you were told. The
     host re-checks that the line named is addressed to this character
     and was marked shareable, so this is not a channel for a phone to
     publish arbitrary text as a quotation. What it *is* a channel for
     is a phone publishing arbitrary text as its own claim, which is
     the point — see TELLING THEM in engine/useGame.js. */
  "shareSecret",
  /* B.3 — the thing to do when it is not your go. One tap, no
     mechanics, no cost, and the only player action in this list
     that can move nothing at all. Rate-limited by the engine, not
     by the network: a limit a phone applies to itself stops
     existing the moment somebody opens the console. */
  "react",
  /* B.4 — "I'm helping her", said by the helper, before the dice.
     The mechanic is old; only the direction is new. Grants nothing
     on its own — the person rolling still has to take it. */
  "offerAssist", "withdrawAssist",
]);
...
export const OUT_OF_TURN = new Set([
  ...
  "pinClue", "unpinClue", "setClueResolved", "addMark", "removeMark",
  "linkClues", "unlinkClues",
  /* Saying something out loud is never a turn, in a firefight least
     of all. Holding it until the shooting stops would make the one
     mechanic whose value is timing into a thing the software times. */
  "shareSecret",
  /* Reacting to a firefight while it is somebody else's go is the
     entire reason this exists. Holding it until the shooting
     stopped would leave the four people not shooting with nothing,
     which is the problem it was written for. */
  "react",
  /* Held out mid-fight is exactly when help is worth offering. */
  "offerAssist", "withdrawAssist",
]);
...
export function packSnapshot({
  seq, phase, mod, g, claims, roster, lobby, safety, table, waiting, mode, ready,
  safetyCall, vote, proposal,
}) {
  return {
    t: H_SNAPSHOT,
    v: PROTOCOL_VERSION,
    ...
    /* How this table is being run. A phone that knows nobody is
       coming can offer to start rather than telling its player to
       wait for a person who does not exist. */
    mode: TABLE_MODES.includes(mode) ? mode : "warden",
    /* Who has ticked ready, by clientId. Only meaningful before a
       session exists, and carries no character data. */
    ready: ready || {},
    claims,
    roster,
    ...
    /* AN OPEN TABLE RULING, if there is one.

       Public to every phone by construction, and that is a
       property of the mechanism rather than an oversight: a fact
       two players agreed to in private and four players do not
       know about is not a secret, it is a faction. So this field
       is never redacted per player and there is no `told` on it —
       see engine/tableRuling.js.

       Carries the sentence and nothing about who said it. The
       proposer's own phone knows it was theirs because it sent the
       message; nobody else needs to, and naming them turns a
       suggestion into a thing to have an opinion about. */
    proposal: proposal ? { text: proposal.text, at: proposal.at } : null,
    ...
    /* THE CARD, ON EVERY PHONE.
       ...
       It used to live only on the host, which meant its only visible
       effect was on somebody else's device. The person who played it
       watched the table carry on and learned that the card does
       nothing — the one lesson a safety tool must never teach.

       It carries the level and the moment, and nothing else. There
       has never been an identity here to leak: the relay strips it
       before the host is told, so there is nothing in this object
       that could name anybody even by accident. */
    safetyCall: safetyCall ? { level: safetyCall.level, at: safetyCall.at } : null,
    /* The table's open question, if it has one. See engine/vote.js.
       Sent whole — every phone needs the tally and the denominator,
       because a vote where you cannot see how many people have not
       answered is a vote you cannot read. */
    vote: vote || null,
...
export function waitingRoom({ game, claims, currentTurn, lastActed = {}, now = Date.now() }) {
  ...
  for (const pc of game.crew) {
    ...
    /* The floor brake, reported the same way as every other one so
       the phone's strip and the Warden's panel agree. `why` is all
       that travels — never the score, never who is waiting. */
    if (floorVerdict({ w: game.w, pcId: pc.id, crew: game.crew, now })) {
      out[pc.id] = { ...base, state: "held", why: "floor" };
      continue;
    }
