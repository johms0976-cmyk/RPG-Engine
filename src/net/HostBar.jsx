...
export default function HostBar({
  ...
  /* How far away the people reading the shared screen are sitting.
     "desk" is the layout this app has always had. "couch" is the
     ten-foot one — see tv.css for the arithmetic. */
  distance = "desk", onDistance = null,
  onJoinCard = null,
  /* Null means there is nothing to assist — the chair is empty and
     the director is not making suggestions, it is running the table. */
  assist = null, onAssist = null,
}) {
  ...
  return (
    <>
      <div className="host-bar">
        ...
        {/* THE ADDRESS, BIG, OVER THE TOP OF EVERYTHING.

            The modal above is sized for somebody standing at this
            machine. It is unreadable from a sofa, which is exactly
            where the person who needs it is sitting — and the
            people who need it most are the ones who arrived late
            or whose phone dropped in act two, so it has to be
            reachable during play and not only from the lobby. */}
        {onJoinCard && <Btn onClick={onJoinCard}>Join card</Btn>}
        {onDistance && (
          <label className="host-distance" title={
            "How far away the people reading the shared screen are. "
            + "The couch layout drops the panels and the log and sizes "
            + "everything off screen height instead."}>
            <span>Screen</span>
            <select value={distance} onChange={(e) => onDistance(e.target.value)}>
              <option value="desk">At a desk</option>
              <option value="couch">Across a room</option>
            </select>
          </label>
        )}
        ...
        {/* THE SWITCH THREE MANIFESTS ASKED FOR.

            Assisted mode has been on for every hosted table since
            2.6.0 and there has never been a way to turn it off. Now
            that `mod.director` actually reaches the engine, that is a
            visible change to how a Warden's table behaves — so it is
            a decision rather than a default, and it sits next to the
            view switcher because that is where a Warden's own
            controls live. */}
        {assist !== null && onAssist && (
          <label className="host-assist" title={assist
            ? "The director is offering suggestions bottom-right. Every one is yours to take or wave away."
            : "The director is not offering anything. Nothing else about the table changes."}>
            <input type="checkbox" checked={!!assist}
              onChange={(e) => onAssist(e.target.checked)} />
