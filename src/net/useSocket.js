...
function useRtcSocket(role, onMessage, opts) {
  ...
  useEffect(() => {
    ...
    /* The host is the hub. It is "open" as soon as it is willing to
       accept peers — there is nobody to connect *to*. Incoming traffic
       is whatever the relay routes up, so the handler is bound onto it
       here; the relay queues anything said before this ran. */
    if (role === "host") {
      if (relay) relay.toHost = (msg) => handler.current && handler.current(msg);
      setStatus(relay ? "open" : "connecting");
      return () => { if (relay) relay.toHost = null; };
    }
