...
export function createRelay({ toHost: initialToHost, peerWhispers = "seen" } = {}) {
  ...
  function fromClient(clientId, msg) {
    ...
    switch (msg.t) {
      ...
      /* The table making a fact true. Port of the same block in
         server/host.mjs and the two must agree — tests/rtcrelay.test.js.
         `asPc` from the router's record, never the message: the
         mechanism is that a proposer cannot second themselves. */
      case "propose":
        if (!c.pcId) return;
        toHost({
          t: "propose", clientId, asPc: c.pcId,
      ...
      case "second":
        if (!c.pcId) return;
        toHost({ t: "second", clientId, asPc: c.pcId });
        return;
