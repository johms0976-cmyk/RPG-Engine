...
export function useHost({ g, mod, phase, lobby, safety, enabled = true }) {
  ...
  useEffect(() => {
    ...
    const full = packSnapshot({
      ...
      lobby,
      safety,
      table: g && g.w && g.w.tableHandout ? { handout: g.w.tableHandout } : null,
    });
  ...
  useEffect(() => {
    ...
    // Held, not refused. Someone else is mid-prompt; try again shortly.
    if (verdict === "wait") {
