...
import {
  noteObjection, emptyObjections, OBJECTION_NOTED, OBJECTION_CARRIED,
} from "../engine/objection.js";
import {
  propose, second, emptyProposals, PROPOSAL_CARRIED,
} from "../engine/tableRuling.js";
...
export function useHost({
  ...
  /* THE OTHER HALF OF THE SAME CORRECTION.
     ...
     `lastDispute` covers Moves addressed to a person. This covers
     the ones addressed to the room, which are most of them. Same
     shape — an object with a timestamp, so a second carried
     objection is a new value rather than an identical one nothing
     notices — and it reaches the ladder by the same route, through
     App.jsx into useDirector. It carries a COUNT and never a name.
     See engine/objection.js. */
  const [lastObjection, setLastObjection] = useState(null);
  /* Who has objected to the current line, and when the group
     started. A ref rather than state: it steers whether the next
     tap carries and must never itself cause a render. */
  const objections = useRef(emptyObjections());
  ...
  const notThatRef = useRef(null);
  const proposeRef = useRef(null);
  const secondRef = useRef(null);
  /* One open proposal at a time. A ref rather than state for the
     same reason `objections` is: the threshold is bookkeeping the
     host does between messages, not something any screen renders. */
  const proposals = useRef(emptyProposals());
  /* A STATE MIRROR OF THE REF, and the reason is the broadcast.

     The ref is what the two callbacks read, because they run
     between renders and must see the latest ledger. But the
     snapshot effect only re-runs on a dependency change, and a ref
     is not one — so an open proposal held purely in a ref would
     never reach the second player's phone, and the mechanism that
     needs two voices would be visible to exactly one of them.

     That is the same class of bug as a feature that passes its
     unit tests and cannot be reached from the running app, which
     this repository has shipped three times. Ref for the logic,
     state for the wire. */
  ...
  const onMessage = useCallback((msg) => {
    ...
    if (msg.t === "notthat") {
      if (notThatRef.current) notThatRef.current(msg.asPc);
      return;
    }
    if (msg.t === "dispute") {
    ...
    /* The table making a fact true. */
    if (msg.t === "propose") {
      if (proposeRef.current) proposeRef.current(msg.asPc, msg);
      return;
    }
    if (msg.t === "second") {
      if (secondRef.current) secondRef.current(msg.asPc);
      return;
    }
  }, [bump]);
  ...
  /* ============================================================
     "NOT THAT" — the same correction, aimed at the room's lines.

     `disputeMove` above is believed instantly and lifts a
     spotlight. This one does neither, because there is nothing to
     lift and nobody to believe: the line was said to everyone. It
     takes a second person, and until it gets one it does exactly
     one thing, which is say out loud that somebody objected.

     That acknowledgement is not decoration. A button that appears
     to do nothing gets pressed once and never again, and this one
     genuinely does nothing on the first press — so it has to say
     so, or the table concludes it is broken and the ladder loses
     the only feedback it has.

     Never a name, in either line. See engine/objection.js.
     ============================================================ */
  const notThat = useCallback((pcId) => {
    if (!pcId) return;
    const game = gRef.current;
    const { next, carried } = noteObjection(objections.current, pcId, Date.now());
    objections.current = next;
    if (game && game.say) game.say("system", carried ? OBJECTION_CARRIED : OBJECTION_NOTED);
    /* Published only when it carried. A lone objection is a thing
       the table heard about and the ladder did not, which is the
       correct asymmetry: one person is not evidence about a line
       five other people were also told. */
    if (carried) setLastObjection({ at: Date.now(), n: 2 });
  }, []);
  notThatRef.current = notThat;

  /* ============================================================
     THE TABLE MAKES A FACT TRUE

     The mirror of `notThat`, and built from the same parts on
     purpose. That one is the table saying a line was wrong; this
     is the table saying a fact is right, and INV-10 gives them the
     same shape because both are addressed to the room.

     Nothing here composes. `msg.text` is the player's own typed
     sentence, it reaches `makeRuling` unmodified, and it is stored
     verbatim — the identical contract the Warden's RulingBox has
     always had. What this decides is WHEN a human's sentence
     becomes true, never what it says.

     Announced on both halves for the reason `notThat` documents: a
     button that appears to do nothing on the first press gets
     pressed once and never again, and this one genuinely does
     nothing on the first press.

     A carried ruling goes through `commitRuling` into world state,
     which puts it in front of `answerLook`, the parser's noun list
     and the transcript at once. That is the whole point — the room
     stops contradicting the table ten minutes later.
     ============================================================ */
  const tablePropose = useCallback((pcId, msg) => {
    if (!pcId) return;
    const game = gRef.current;
    if (!game) return;
    const { next, said, error } = propose(
      proposals.current,
      {
        text: msg.text,
        scope: msg.scope,
        /* WHERE they are, from the host's own state rather than
           from the phone. "This room" means the room the character
           is standing in, and a phone does not get to nominate a
           different one — `here` is the same lookup every other
           room-aware host path uses now that the party can split. */
        room: (game.here && game.here(pcId)) || (game.w && game.w.room) || null,
        subject: msg.subject,
        by: pcId,
        clock: (game.w && game.w.clock) || 0,
      },
      Date.now(),
    );
    if (error) {
      /* Back to the one phone. A malformed proposal is that
         player's problem and not a thing to read to the room. */
      if (sendRef.current) sendRef.current({ t: "denied", to: pcId, reason: error });
      return;
    }
    proposals.current = next;
    setOpenProposal(next.open ? { text: next.open.ruling.text, at: next.open.at } : null);
    if (game.say) game.say("system", said);
  }, []);
  proposeRef.current = tablePropose;

  const tableSecond = useCallback((pcId) => {
    if (!pcId) return;
    const game = gRef.current;
    if (!game) return;
    const { next, carried, ruling, error } = second(proposals.current, pcId, Date.now());
    proposals.current = next;
    setOpenProposal(next.open ? { text: next.open.ruling.text, at: next.open.at } : null);
    if (error) {
      if (sendRef.current) sendRef.current({ t: "denied", to: pcId, reason: error });
    ...
    if (!carried || !ruling) return;

    /* THROUGH THE WARDEN'S OWN DOOR.

       `game.warden.rule` is the path RulingBox has always used. It
       says the ruling first and stores it second, in that order and
       for the reason documented in wardenRulings.js, and it keeps
       the append rule in exactly one place.

       Reusing it rather than calling `commitRuling` here is the
       same discipline that put `LADDER` behind `RUNGS`: a second
       commit path would drift from the first, and the drift would
       be silent because both would still work.

       `by: "table"` is the only difference, and it is the one that
       matters — the transcript and the Warden's ledger both show
       who made a fact true, and "the table" is a different answer
       from any player's name. */
    if (game.warden && game.warden.rule) {
      game.warden.rule(ruling.text, {
        scope: ruling.scope,
        subject: ruling.subject || undefined,
        room: ruling.room || undefined,
        by: "table",
      });
    }
    if (game.say) game.say("system", PROPOSAL_CARRIED);
  }, []);
  secondRef.current = tableSecond;
  ...
  return {
    ...
    peerDowngrade,
    sound, cue, spotlight, spotlightPeer, lastDispute, lastObjection,
