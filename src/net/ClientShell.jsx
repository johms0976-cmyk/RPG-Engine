...
import { ReactBar, AssistOffer } from "../ui/ReactBar.jsx";
import { REACT_COOLDOWN_MS } from "../engine/reactions.js";
import MakeItTrue from "./MakeItTrue.jsx";
import { useAwayMark } from "./useAwayMark.js";
import { buildRecap } from "../engine/tempo.js";
...
import { WhoseGo, SafetyBanner, TableVote, AskRoom } from "./TableControls.jsx";
...
import SecretPocket from "../ui/SecretPocket.jsx";
import { heldSecrets } from "../engine/secrets.js";
...
export default function ClientShell({ rtcLink = null }) {
  /* THE TWO THINGS A HANDSET DECLARES ABOUT ITSELF.

     Sound: off, by rule, before anything can ask for it. See
     audio.setRole — the shared screen owns the room's noise and
     this device owns the buzz in a pocket.

     Screen: awake for as long as this phone is at the table. The
     spotlight is worth nothing if it lands on a locked handset,
     and before this the most common interruption of an evening
     was somebody unlocking a phone to find out why it buzzed. */
  useEffect(() => { audio.setRole("phone"); }, []);
  useWakeLock(true);
  ...
  const { status, send } = useSocket(
    ...
    onMessage,
    undefined,
    /* A remote phone carries its own link in; a LAN phone dials the
       relay exactly as before. Nothing below this line knows which. */
    rtcLink ? { transport: "rtc", link: rtcLink } : undefined,
  );
  ...
  /* WHAT YOU MISSED WHILE YOU WERE OUT. The mark is set by the
     phone going dark and read when it comes back — see
     net/useAwayMark.js for why it is not a button. The card is
     built from this client's OWN feed, which is already redacted,
     so a catch-up can never show somebody something they were not
     told in the first place. */
  const away = useAwayMark(g && g.feed, inPlay);
  const awayRecap = useMemo(() => {
    if (away.sinceId == null || !g || !g.feed) return null;
    return buildRecap({
      feed: g.feed, crew: g.crew || [], mod: g.mod, w: g.w, sinceId: away.sinceId,
    });
  }, [away.sinceId, g && g.feed, g && g.crew, g && g.mod, g && g.w]);
  ...
  /* The card and the ballot, both off the snapshot rather than held
     locally — the host is the authority on whether the table is
     paused, exactly as it is for everything else. */
  const safetyCall = (snapshot && snapshot.safetyCall) || null;
  const tableVote = (snapshot && snapshot.vote) || null;
  const myVote = (tableVote && clientId && tableVote.cast) ? tableVote.cast[clientId] : null;
  /* With nobody behind the screen, the decisions a Warden used to
     make have to be reachable from here. With a Warden present they
     are the Warden's, and this does not render. */
  const wardenless = (snapshot && snapshot.mode) === "wardenless";
  ...
  /* Everything this player was told and has not spoken about. Derived
     from the feed they already have — the snapshot was redacted host
     side, so a phone can only ever compute its own. */
  const secrets = React.useMemo(
    () => (g && g.feed && pcId ? heldSecrets(g.feed, pcId) : []),
    [g && g.feed, pcId],
  );

  /* B.3/B.4. Both go down the ordinary intent channel, which is
     why neither needed a line of router surgery: `intent` already
     validates against PLAYER_ACTIONS on the server AND again on
     the host, and already attaches the character from the
     server's own record rather than trusting the phone. */
  const react = useCallback((kind) => {
    if (!pcId) return;
    gateRef.current.send({ t: "intent", action: "react", args: [kind], asPc: pcId });
    /* Local, and optimistic only about the button. The engine owns
       the real cooldown; this just stops the six buttons looking
       live for the second before the snapshot comes back. */
    setCooling(Date.now());
  }, [pcId]);

  const offerAssist = useCallback((toPcId) => {
    if (!pcId) return;
    gateRef.current.send({ t: "intent", action: "offerAssist", args: [toPcId], asPc: pcId });
  }, [pcId]);

  const withdrawAssist = useCallback(() => {
    if (!pcId) return;
    gateRef.current.send({ t: "intent", action: "withdrawAssist", args: [], asPc: pcId });
  }, [pcId]);

  const shareSecret = useCallback((lineId, mode, text) => {
    if (!gateRef.current) return;
    gateRef.current.send({ t: "intent", action: "shareSecret", args: [lineId, mode, text || ""], asPc: pcId });
  }, [pcId]);
  ...
  /* And down again, from whichever phone. Also raw, and for the same
     reason inverted: if the clear carried an identity the host would
     learn who took the card down, which on a table where the card was
     about one person is nearly as identifying as knowing who put it
     up. See C_CLEARSAFETY. */
  const clearSafety = useCallback(() => {
    send({ t: "clearsafety" });
  }, [send]);

  /* The table deciding something. Stamped, unlike the card — a ballot
     with anonymous papers cannot tell whether everybody has answered,
     and that is exactly what makes silence count as no. */
  const castVote = useCallback((choice) => {
    send({ t: "vote", choice });
  }, [send]);

  const openVoteTopic = useCallback((topic) => {
    send({ t: "openvote", topic });
  }, [send]);

  /* "What do I see?" — answered privately from what this character
     can already know. See `answerLook` in useHost. */
  const askRoom = useCallback((about) => {
    send({ t: "look", about });
  }, [send]);

  /* THE TWO HALVES OF "NOT THAT".

     `notThat` is about the last thing said to the ROOM and takes a
     second person to carry — most of the empty chair's output is
     unaddressed, and none of it could be corrected before this.

     `notMe` is about the last thing aimed at YOU and is believed
     at once. The message for it has existed since 2.7.0, the relay
     has always forwarded it and the host has always acted on it,
     and no screen in the application ever sent it — so the one
     correction the design leaned on hardest was, in practice,
     unpressable. It hangs off the spotlight because that is the
     moment the empty chair is unambiguously pointing at you.

     See engine/objection.js. */
  const notThat = useCallback(() => { send({ t: "notthat" }); }, [send]);
  /* THE TABLE MAKING A FACT TRUE. See engine/tableRuling.js and
     net/MakeItTrue.jsx. Same shape as `notThat` on purpose — both
     are addressed to the room and both take a second voice. */
  const propose = useCallback((spec) => { send({ t: "propose", ...spec }); }, [send]);
  const secondIt = useCallback(() => { send({ t: "second" }); }, [send]);
  const notMe = useCallback(() => { send({ t: "dispute" }); }, [send]);

  let body;
  if (offer) {
    body = (
      <OfferStatus offer={offer} onWithdraw={withdraw}
        wardenless={(snapshot && snapshot.mode) === "wardenless"}
        ready={!!(snapshot && snapshot.ready && clientId && snapshot.ready[clientId])}
        onReady={(v) => send({ t: "ready", ready: v })}
    ...
    /* `onAgain` is null rather than a no-op: starting the module over
       is the host's decision and a phone has never been able to make
       it, so a button that silently did nothing was worse than none. */
    body = (
      <Ending
        mod={g.mod} w={g.w} crew={g.crew} feed={g.feed}
        phone pcId={pcId} onAgain={null} onLibrary={() => setPcId(null)}
      />
    );
  } else if (!g.pc) {
    body = (
      <div className="join">
        <Panel title="You're out">
          {/* This used to say "the Warden can bring in someone new",
              which stopped being true the moment the empty chair
              shipped and was never true for the player reading it —
              they had no way to ask. Now it is a door rather than a
              description of somebody else's options. */}
          <p>Your character is no longer in play.</p>
          <div className="btn-grid">
            <Btn kind="primary" onClick={() => setBuilding(true)}>Make someone new</Btn>
            <Btn kind="ghost" onClick={() => setPcId(null)}>Pick another</Btn>
          </div>
        </Panel>
      </div>
    );
  ...
  return (
    <ThemeProvider theme={mod.theme} treatment={mod.theme.treatment} feedStyles={mod.feedStyles}>
      <ErrorBoundary mod={mod} onEject={() => setPcId(null)}>
        ...
        {banner}
        {/* THE CARD, WHERE ITS OWNER CAN SEE IT.

            Above everything, including the held strip, because the
            held strip will be reporting the hold this caused and the
            cause should be read first. Any phone can take it down —
            see SafetyBanner. */}
        {safetyCall && (
          <SafetyBanner call={safetyCall} onClear={clearSafety} />
        )}
        {/* The table's open question, if it has one. */}
        {tableVote && (
          <TableVote vote={tableVote} mine={myVote} onCast={castVote} />
        )}
        {/* Who the table is actually waiting on. With a Warden this is
            a person looking at you; without one it has to be a
            sentence, or nobody speaks. */}
        {inPlay && (
          <WhoseGo
            waiting={(snapshot && snapshot.waiting) || null}
            pcId={pcId}
            crew={g.crew}
          />
        )}
        ...
        {/* AND IT STAYS OFFERED.

            The takeover card is a moment and moments are dismissible —
            somebody who just lost a character they liked is entitled to
            close it and sit with that for twenty minutes. But an offer
            that only exists inside a modal you have already shut is not
            an offer, and the whole failure this fixes is a player with
            nothing on their phone. So the door stays visible for as
            long as it is true. */}
        {inPlay && g.pc && g.pc.alive === false && (
          <div className="net-strip is-dead" role="status">
            <span>{g.pc.name} is gone.</span>
        ...
        {/* Quiet, and on one phone. It is a count in a corner until it
            is touched, because the whole value of the thing it holds
            is that nobody can tell you are holding it. */}
        {inPlay && <SecretPocket secrets={secrets} onShare={shareSecret} />}
        {/* THE DEAD TIME. Four people watching one person act is
            where a party game is lost, and until now every answer
            this engine had was a *move* — something that costs
            thought and arrives with consequences. These two cost
            nothing, which is the point. */}
        {inPlay && (
          <ReactBar
            onReact={react}
            cooling={cooling && Date.now() - cooling < REACT_COOLDOWN_MS}
            disabled={g.pc.alive === false}
          />
        )}
        {inPlay && g.pc.alive !== false && (
          <AssistOffer
            crew={g.crew || []}
            me={pcId}
            offered={(g.assistOffers || []).find((o) => o.by === pcId) || null}
            spent={g.pc.lastAssistDay != null && g.pc.lastAssistDay === (g.w || {}).day}
            onOffer={offerAssist}
            onWithdraw={withdrawAssist}
          />
        )}
        {recap && <RecapCard recap={recap} onClose={() => setRecapDown(recap.at)} />}

        {/* The personal one, beneath the table's. Both can be open
            at once and that is fine — they answer different
            questions ("where are we" vs "what did I miss"). */}
        {awayRecap && (
          <RecapCard
            recap={{ ...awayRecap, title: `While you were away — ${away.missed} lines` }}
            onClose={away.clear}
          />
        )}
        ...
        {/* THE DOOR OUT OF BEING DEAD.

            `setBuilding(true)` is the same route the lobby uses, so a
            replacement character travels the path that is already
            tested end to end: CreatorPhone -> `submit` -> the Warden's
            queue (or straight through with the chair empty, because
            there is nobody to approve it) -> `addCrewMember`, who
            arrives where the crew is -> `assigned`, which repoints
            this handset at the new body and drops the claim on the old
            one by itself.

            Offered only when this character is actually dead. Going
            down at 0 Health is not death — the Warden has already
            rolled for when you come back — and offering somebody a new
            body while their old one is merely unconscious would be the
            engine telling them something the Warden is deliberately
            keeping to themselves. */}
        <DeathTakeover
          event={death}
          onDismiss={() => setDeath(null)}
          onNewCharacter={g && g.pc && g.pc.alive === false
            ? () => { setDeath(null); setBuilding(true); }
            : null}
          arrival={(mod && mod.replacement && mod.replacement.arrival) || null}
        />
        <ClassAlert alert={classFx} onDismiss={() => setClassFx(null)} />
        <Spotlight
          spot={spot}
          onDone={() => setSpot(null)}
          onNotMe={() => { notMe(); setSpot(null); }}
        />
        ...
        {/* ============================================================
            ...
            Only when nobody is the Warden, and only in play. These are
            the things a person behind the screen supplied for free:
            asking the situation a question, and calling the four
            decisions that are not rules decisions at all.

            Deliberately at the bottom, small, and never modal. A table
            that is going well should be able to forget these exist —
            they are the fire exit, not the furniture.
            ============================================================ */}
        {inPlay && wardenless && !safetyCall && (
          <div className="table-bar">
            <AskRoom onAsk={askRoom} disabled={status !== "open"} />
            {!tableVote && (
              <>
                <Btn kind="default" onClick={() => openVoteTopic("breather")}>Take five</Btn>
                <Btn kind="default" onClick={() => openVoteTopic("rewind")}>Rewind that</Btn>
                {/* The floor, reachable from the only screens a
                    wardenless table has. See VOTE_TOPICS.floor. */}
                <Btn kind="default" onClick={() => openVoteTopic("floor")}>Share the floor</Btn>
                {/* Not a vote, on purpose: a table stopping to hold a
                    referendum on a sentence has already lost more to
                    the interruption than the sentence cost. One tap,
                    anonymous, and it needs a second one from somebody
                    else. See engine/objection.js. */}
                <Btn kind="default" onClick={notThat}>Not that</Btn>
                {/* The one control here that is not a signal. The
                    empty chair cannot reward an idea the module never
                    anticipated (see director.js's own header); this
                    is how the table does it instead, and it is the
                    reason ruling.js is no longer Warden-only. */}
                <MakeItTrue
                  onPropose={propose}
                  onSecond={secondIt}
                  open={snapshot && snapshot.proposal}
                  disabled={status !== "open"}
                />
                <Btn kind="default" onClick={() => openVoteTopic("callit")}>Call it</Btn>
              </>
            )}
          </div>
        )}
...
export function OfferStatus({
  offer, onWithdraw, onAgain, onResend,
  wardenless = false, ready = false, onReady = () => {}, onStart = () => {},
}) {
  const pc = offer.pc || {};
  ...
  /* THE WAITING ROOM WHEN THERE IS NOBODY TO WAIT FOR.

     "They'll wave you in when they've had a look" is a lie at an
     empty-chair table and, worse, it is a lie that tells the player
     to sit still. Nobody is coming. So the copy changes and the
     phone grows the two controls the missing person used to be:
     ready, and go. */
  if (wardenless) {
    return (
      <div className="join">
        <Panel title="Everybody in?">
          <div className="stack">
            <p style={{ margin: 0 }}>
              <strong>{pc.name}</strong> is at the table. Nobody is running this
              one — it starts when somebody says so, and that can be you.
            </p>
            ...
            <div className="btn-row">
              <Btn kind={ready ? "accent" : "default"} onClick={() => onReady(!ready)}>
                {ready ? "Ready" : "I'm ready"}
              </Btn>
              <Btn kind="primary" onClick={onStart}>Start it</Btn>
            </div>
            <p className="clue-meta" style={{ margin: 0 }}>
              Starting before everyone has a character does nothing — check the
              big screen first.
            </p>
