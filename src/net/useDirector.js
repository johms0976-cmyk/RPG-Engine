...
import { directorPlan, VETO_LIMIT, CALLBACK_PREFIX } from "../engine/director.js";
...
export function useDirector({
  ...
  /** Did this Move make the evening worse for the people at the
      table? The list is short and deliberately literal: a screw
      turned, a fight started, somebody tested, a failure narrated,
      the creature moved. Everything else — describing a room,
      passing the floor, reading a clock — is neutral or kind, and a
      table does not need a rest from it. */
  const HARSH = new Set(["escalate", "combat", "callRoll", "pressure"]);
  const isHarsh = (m) => !!(m && (HARSH.has(m.kind)
    || (m.kind === "describe" && m.rung === "aftermath")));
