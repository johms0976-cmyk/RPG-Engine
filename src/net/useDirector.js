...
import { OBJECTION_VETOES } from "../engine/objection.js";
...
/* ------------------------------------------------------------
   ...
   `api.sayIn` resolves the audience through `party.js` and hands a
   redacted line to exactly the phones whose characters are standing
   there. It has been the module layer's verb for this since per-PC
   rooms landed; the director was written before that and kept
   calling `warden.say`, which addresses the whole table.

   The fallback is not defensive padding — a Move with no room (a
   clock, a nudge, anything about the table rather than the fiction)
   is *correctly* addressed to everybody, and so is any line at all
   on a game object old enough not to carry an api.
   ------------------------------------------------------------ */
function speak(game, room, tone, text, extra) {
  if (!game || !text) return;
  if (room && game.api && game.api.sayIn) { game.api.sayIn(room, tone, text, extra); return; }
  if (game.warden) game.warden.say(text, tone, extra);
}
...
export function useDirector({
  ...
  dispute = null,
  /* The table's own "not that", already past its quorum by the
     time it arrives here — `useHost` holds the tally and only
     publishes what carried. `{ at, n }`, never a name. */
  objection = null,
  ...
  /* ---------------- when a PLAYER last did something -------------

     `rungPressure` has always taken a `lastActedAt` and this hook has
     never passed one. `directorPlan` defaults it to 0, and the rung
     opens with `if (!lastActedAt) return null`, so the pressure rung
     could not fire in the shipped app — not once, in any module, at
     any stall length. Dead Weight's `director.pressure` is `prowl`,
     the module's only threat movement, which is why a wardenless
     table got scenery and nothing else no matter how long it sat.

     Read off `w.clock` rather than the feed. `lastLineAt` already
     tracks the feed, and the feed is the wrong signal here for the
     exact reason this rung exists: the director's own atmosphere
     lines are feed lines, so a stalled table would keep resetting
     its own stall timer with the scenery it was being given instead
     of pressure. The fiction clock only moves when somebody acts,
     which is precisely the definition this rung wants. */
  ...
  /* WHEN EACH ROOM WAS LAST THE SUBJECT OF A MOVE.

     `focusRoom` rotates on this, so a split party gets attention in
     turn rather than the majority getting all of it. Stamped on the
     way OUT, in `take`, for the same reason `harshAt` is: what a
     room was actually given, not what was offered and waved away. */
  const roomServedAt = useRef({});
  ...
  /* When the director last spent fiction-minutes for the table.
     `rungPacing` reads it to space the skips out; nothing else
     touches it. A ref for the same reason as the rest of these —
     it steers the next plan and must never cause a render. */
  ...
  /* THE LAST THING SAID, ADDRESSED OR NOT.

     `aimedAt` is deliberately null for a Move with no `pcId`, and
     that was the whole of F6: `describe`, `atmosphere`, `npcSay`,
     `clock` and `callback` carry no player, so the ledger could
     never learn anything about the rungs that emit them — which
     are most of the rungs and nearly all of the output.

     This is the same record without the addressee, and it exists
     for exactly one consumer: a table-level objection, which is
     about a line rather than about a person and therefore has no
     `pcId` to match on. It matches on time instead. */
  const lastTaken = useRef(null);
  const seenObjection = useRef(0);
  const objections = useRef({});
  ...
  /* A CARRIED OBJECTION, against whatever was last said.
   *
   * Same window as a dispute and the same silence on a miss: an
   * objection that arrives long after the last Move is about
   * something a person said, or about nothing, and the ladder has
   * no business guessing which.
   *
   * Weighted in VETOES rather than in disputes. Two people
   * agreeing about a line said to both of them is at least as good
   * evidence as one Warden's veto, and better than one player
   * waving off one moment aimed at them — which is why it does not
   * go through DISPUTE_WEIGHT. See engine/objection.js.
   */
  useEffect(() => {
    if (!objection || !objection.at || objection.at === seenObjection.current) return;
    seenObjection.current = objection.at;
    const last = lastTaken.current;
    if (!last || !last.rung) return;
    /* Absolute, not one-sided. An objection stamped well BEFORE
       the last Move is about the Move before it — a line the
       ladder has already moved on from — and counting it would
       attribute a complaint to whichever rung happened to speak
       next. Two hosts' clocks are also never exactly equal, so a
       few milliseconds either way has to be tolerated; ten
       minutes either way does not. */
    if (Math.abs(objection.at - last.at) > DISPUTE_WINDOW_MS) return;
    objections.current[last.rung] = (objections.current[last.rung] || 0) + OBJECTION_VETOES;
  }, [objection]);
  ...
  const ledger = useCallback(() => {
    ...
    /* Already counted in vetoes on the way in, so it is added
       rather than divided. Same ledger as the other two, for the
       reason stated above: three parallel mechanisms for "the
       table said no" is two more than anybody could reason about
       later. */
    for (const [rung, n] of Object.entries(objections.current)) {
      out[rung] = (out[rung] || 0) + n;
    }
    return out;
  }, []);
  ...
  /* The fiction clock moving IS a player acting — nothing else turns
     it, which is the whole of §6's complaint about idle tables. */
  const clockNow = g && g.w ? g.w.clock : null;
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    /* Seeded, not left at zero. A table that has not acted *yet* has
       still been sitting there since the session opened, and the stall
       this rung measures should run from the moment the game started
       rather than from the first thing anybody happened to do. */
    if (!lastActedAt.current) lastActedAt.current = Date.now();
  }, [enabled]);
  ...
  const take = useCallback((m) => {
    ...
    /* And the same thing without the addressee, which is what a
       table-level "not that" matches against. Every Move, aimed or
       not — the unaddressed ones are the entire point. */
    lastTaken.current = { rung: m.rung || null, kind: m.kind, at: Date.now() };

    /* This room has now had its turn. */
    if (m.room) roomServedAt.current = { ...roomServedAt.current, [m.room]: Date.now() };
    ...
    switch (m.kind) {
      case "describe":
        /* `interject` is the register a person's own narration lands
           ...
           SAID IN A ROOM, NOT AT A TABLE. `sayIn` is the module's own
           verb for this and it has existed since the party could
           split; the director simply never used it, so six people in
           two rooms all heard whichever room the majority was in.
           `audienceFor` returns null while everybody is together, so
           an unsplit table gets a byte-identical public line and
           cannot tell this changed. */
        speak(game, m.room, "interject", m.text, { director: m.rung });
      ...
      case "spotlight":
        ...
        return;
      case "passTime":
        /* THE ONE MOVE THAT CHANGES THE WORLD WITHOUT ANYBODY ACTING.

           `warden.passTime` is the same call the deck's button makes,
           and it is deliberately the ONLY route: it already runs the
           module's own `onTick` across the interval, drops countdowns
           by the right amount and emits the engine's neutral marker,
           so nothing here composes a sentence and every module's own
           scheduled beat still fires on the way through.

           Reaching for `game.advance` instead would move the number
           and skip the module, which is the silent-desync bug this
           whole rung exists to prevent. */
        if (game.warden && typeof game.warden.passTime === "function") {
          game.warden.passTime(m.mins);
          /* And then the second half of the beat: having spent the
             time, say what it cost. This is what the old `clock`
             Move did on its own — worth keeping, but as the
             consequence of turning the clock rather than as a
             substitute for it. */
          if (m.countdown != null) {
      ...
      case "combat":
        /* IT COMES THROUGH THE DOOR.
           ...
           not ours. */
        if (game.warden) {
          if (m.reason) speak(game, m.room, "interject", m.reason, { director: m.rung });
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const next = directorPlan({
        ...
        lastActedAt: lastActedAt.current,
        ...
        vetoes: ledger(),
        roomServedAt: roomServedAt.current,
        heard: heard.current,
        sessionEndsAt,
        lastCallAt: lastCallAt.current,
        lastPassAt: lastPassAt.current,
      });
  ...
  return {
    move, take, dismiss,
    vetoes: vetoes.current, disputes: disputes.current, objections: objections.current,
